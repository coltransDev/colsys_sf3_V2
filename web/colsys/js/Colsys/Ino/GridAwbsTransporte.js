                ...
                if (form.isValid()) {
                    Ext.Ajax.request({
                        ...
                        success: function (response, options) {
                            win = me.findParentByType('window');
                            store = Ext.getCmp(win.getId().replace("winAwbsTransporte", "Impresion")).getStore();
                            store.reload();
                            win.close();
                        }
                    });
            ...
            handler: function () {
                ...
                if (form.isValid()) {
                    Ext.Ajax.request({
                        ...
                        success: function (response, options) {
                            win = me.findParentByType('window');
                            store = Ext.getCmp(win.getId().replace("winLiquidDoc", "Impresion")).getStore();
                            store.reload();
                            win.close();
                        }
                    });
...
var gridHawbs = Ext.create('Ext.grid.Panel', {
    ...
    dockedItems: [{
            ...
            items: [{
                    text: 'Adicionar',
                    tooltip: 'Nuevo Documento de Hijo',
                    iconCls: 'add',
                    handler: function () {
    ...
    listeners: {
        ...
        beforerender: function (ct, position) {
                                    ...
                                    handler: function (grid, rowIndex, colIndex) {
                                        ...
                                        if (win_header == null) {
                                            win_header = new Ext.Window({
                                                id: 'winAwbsTransporte-' + me.idmaster,
                                        ...
                                        Ext.getCmp('idcarrier_uno').store.add(
                                                {"idcarrier": rec.data.idcarrier_uno, "carrier": rec.data.carrier_uno}
                                        );
                                        Ext.getCmp('idcarrier_dos').store.add(
                                                {"idcarrier": rec.data.idcarrier_dos, "carrier": rec.data.carrier_dos}
                                        );
                                        Ext.getCmp('idcarrier_trs').store.add(
                                                {"idcarrier": rec.data.idcarrier_trs, "carrier": rec.data.carrier_trs}
                                        );
            ...
            //if (this.permisos.Crear == true || this.permisos.Editar == true) {
                ...
                tb.add({
                    text: 'Adicionar',
                    tooltip: 'Nuevo Documento de Transporte',
                    iconCls: 'add',
                    scope: this,
                    handler: function () {
                        var rec = Ext.create('GuiaMaster', {});
                        rec.set('idmaster', this.idmaster);
                        if (win_header == null) {
                            win_header = new Ext.Window({
                                id: 'winAwbsTransporte-' + this.idmaster,
                                title: 'Cabecera del Documento',
                                width: 650,
                                closeAction: 'close',
                                items: {
