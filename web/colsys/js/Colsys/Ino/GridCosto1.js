            ...
            this.store.reload({
                callback: function (records, operation, success) {
                    for (i = 0; i < records.length; i++) {
                        ...
                        console.log(Ext.ComponentQuery.query("#costo" + idmaster));
                        //if( Ext.ComponentManager.get("costo" + idmaster))
                        {
                            ...
                            console.log(nombrecos+"::"+idcos);
                            console.log(rec.data.idcosto+"--"+rec.data.nombrecosto);
                            //console.log(rec.data.nombrecosto);
                            Ext.getCmp("costo" + idmaster).store.add(
                                {"id": rec.get("idcosto"+idmaster), "name": rec.get("nombrecosto"+idmaster)}
                            );
                            rec.set(("costo" + idmaster), rec.data.idcosto);
                            rec.set(("idcosto" + idmaster), rec.data.nombrecosto);
                        }
                ...
                success: function (response, opts) {
                    ...
                    if (obj.root) {
                        for (i = 0; i < obj.root.length; i++) {
                            ...
                            //console.log(obj)
                            eval("var nombrecos=obj.root[i].nombrecosto" + idmaster + ";var idcos=obj.root[i].idcosto" + idmaster + ";");
                            //console.log(nombrecos);
                            //console.log(idcos);
                            Ext.getCmp('comboCosto' + idmaster).store.add(
                                    {"id": idcos, "name": nombrecos}
                            );
                            //Ext.getCmp('comboCosto' + idmaster).setValue(idcos);
                            //alert(idcos)
                        }
            ...
            this.reconfigure(
                    store = Ext.create('Ext.data.Store', {
                        fields: [
                            ...
                            {name: 'costo' + this.idmaster, type: 'string'},
