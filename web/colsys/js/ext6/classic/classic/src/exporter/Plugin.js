...
Ext.define('Ext.exporter.Plugin', {
    ...
    init: function (cmp) {
        ...
        cmp.saveDocumentPdfAs = Ext.bind(me.saveDocumentPdfAs, me);
    ...
    destroy: function(){
        ...
        cmp.saveDocumentAs = cmp.saveDocumentPdfAs = cmp.getDocumentData = me.cmp = me.delayedSaveTimer =
    ...
    saveDocumentPdfAs: function(cfg){
        var exporter;
        ...
        var records = this.getCmp().getStore().getRange();

        for(i=0;i<records.length; i++){
            if(records[i].data.ca_color)
                colors[i] = records[i].data.ca_color;
            else
                colors[i] = "white";
        }
        ...
        exporter = this.getExporterPdf.apply(this, arguments);
        
        for(var k=0; k < exporter.data._columns.items.length; k++) {            
            exporter.data._columns.items[k]= {text: exporter.data._columns.items[k]._text, style: "tableHeader"};            
        }
        
        tData.push(exporter.data._columns.items);
        
        
        var dataRowLength = exporter.data._rows.items.length;
        for(var i=0; i < dataRowLength; i++){
            var columnLength = exporter.data._rows.items[i]._cells.items.length;
            for(var j=0; j < columnLength; j++){
                exporter.data._rows.items[i][j] = exporter.data._rows.items[i]._cells.items[j]._value;
                if(exporter.data._rows.items[i]._cells.items[j]._value)
                    valor[j] = {text: exporter.data._rows.items[i]._cells.items[j]._value, fillColor: colors[i]};
                else
                    valor[j] = {text: "", fillColor: colors[i]};
            }            
            tData.push(valor);
            valor = [];
        }
    ...
    getExporterPdf: function(config){
