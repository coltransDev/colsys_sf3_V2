...
//	include_once 'include/seguridad.php';                             // Control de Acceso al módulo
    ...
    echo "</TABLE>";
    echo "<TABLE CELLSPACING=10>";
    echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='accion' VALUE='Regresar' ONCLICK='javascript:document.location.href = \"grasucursalfcl.php\"'></TH>";  // Cancela la operación
    echo "</TABLE>";
    echo "</CENTER>";
} elseif (!isset($boton) and isset($accion) and isset($sucursal)) {
...
//  include_once 'include/seguridad.php';                                      // Control de Acceso al módulo
    ...
    if (!$fc->Open("select substr(ca_mes,4,1) as ca_ano, ca_sucursal, ca_capacidad, sum(ca_cantidad) as ca_cantidad from vi_inocarga_fcl $condicion group by ca_ano, ca_sucursal, ca_capacidad order by ca_sucursal, ca_capacidad")) {                       // Selecciona todos lo registros de la tabla Ino-Marítimo
