...
// Updated:       2004-05-11                                                  \\
//                                                                            \\
// Descripción:   Reporte sobre Movimiento de Carga por Sucursales            \\
//                                                                            \\
//                                                                            \\
...
$titulo = 'Movimiento de Carga LCL por Sucursales';
...
require_once("menu.php");
    ...
    echo "<CENTER>";
    echo "<H3>$titulo</H3>";
    echo "<FORM METHOD=post NAME='menuform' ACTION='grasucursallcl.php'>";
...
require_once("menu.php");
	...
	echo "<TR>";
	echo "  <TD Class=invertir><iframe ID=consulta_tar src ='grasucursallcl.php?accion=Imprimir&ano=$ano&mes=$mes&sucursal=$sucursal' width='100%' height='100%'></iframe></TD>";
	echo "</TR>";
	echo "</TABLE>";
	echo "<TABLE CELLSPACING=10>";
	echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='accion' VALUE='Regresar' ONCLICK='javascript:document.location.href = \"grasucursallcl.php\"'></TH>";  // Cancela la operación
	echo "</TABLE>";
	echo "</CENTER>";
    }
elseif (!isset($boton) and isset($accion) and isset($sucursal)){
...
//  include_once 'include/seguridad.php';                                      // Control de Acceso al módulo
    ...
    if (!$lc->Open("select substr(ca_mes,4,1) as ca_ano, ca_sucursal, sum(ca_volumen) as ca_cantidad from vi_inocarga_lcl $condicion group by ca_ano, ca_sucursal order by ca_sucursal")) {                       // Selecciona todos lo registros de la tabla Ino-Marítimo
	...
	while (!$lc->Eof() and !$lc->IsEmpty()) {
	 	if (!in_array($lc->Value('ca_sucursal'), $traficos)) {
	 		array_push($traficos, $lc->Value('ca_sucursal'));
