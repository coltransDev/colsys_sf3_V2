...
//			echo "<P DIR='RTL'><A HREF=\"#\" ONCLICK='javascript:window.open(\"./help/$modulo.html\",\"Ayuda\",\"scrollbars=yes,width=600,height=400,top=200,left=150\")'><IMG SRC='./graficos/help.gif' border=0 ALT='Ayuda en Línea'><BR>Ayuda</A></P>";  // Link que proporciona la Ayuda en línea
	...
	$tm =& DlRecordset::NewRecordset($conn);
	if (!$tm->Open("select ca_idrecargo, ca_recargo, r.ca_transporte from tb_tiporecargo r, vi_cotproductos p where r.ca_tipo = 'Recargo en Origen' and r.ca_transporte = p.ca_transporte and fun_texttokencompare(r.ca_incoterms ,p.ca_incoterms, '|') and r.ca_tipo = '$tp' and ca_idproducto = '$pr' order by ca_recargo")) {       // Selecciona todos lo registros de la tabla Traficos
		echo "<script>alert(\"".addslashes($tm->mErrMsg)."\");</script>";      // Muestra el mensaje de error
		echo "<script>document.location.href = 'cotizaciones.php';</script>";
		exit; }
	$tm->MoveFirst();
...
//  Definición de Conceptos
    ...
    $mn =& DlRecordset::NewRecordset($conn);
    $co =& DlRecordset::NewRecordset($conn);
    if (!$rc->Open("select ca_idrecargo, ca_recargo from tb_tiporecargo where ca_tipo = 'Recargo Local' and ca_transporte = '".$rs->Value('ca_transporte')."' and ca_incoterms != '' and fun_texttokencompare(ca_incoterms, '".$rs->Value('ca_incoterms')."','|') order by ca_recargo")) {   // Selecciona todos lo registros de la tabla Recargos
        echo "<script>alert(\"".addslashes($rc->mErrMsg)."\");</script>";      // Muestra el mensaje de error
        echo "<script>document.location.href = 'reportenegocio.php';</script>";
        exit; }
    $rc->MoveFirst();
...
//  Definición de Conceptos
    ...
    while (!$rs->Eof() and !$rs->IsEmpty()) {                                                      // Lee la totalidad de los registros obtenidos en la instrucción Select
	   if ( $rs->Value('ca_idcliente') != $id_temp ) {
        ...
        $pdf->Row(array("¿Ofrece algún peligro para Coltrans? :",$rs->Value('ca_peligro'), "Explique :", $rs->Value('ca_explicacion')));
	...
	if (!$rs->IsEmpty()) {
		...
		switch($extension){
			...
			case "tif":     $type = "image/x-tiff";                  break;
			case "tiff":    $type = "image/tiff";                    break;
			case "tiff":    $type = "image/x-tiff";                  break;
