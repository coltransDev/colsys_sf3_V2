...
if (!isset($boton) and !isset($accion) and !isset($traorigen)){
    echo "<HTML>";
    echo "<HEAD>";
...
require_once("menu.php");
    ...
    echo "<CENTER>";
    echo "<H3>$titulo</H3>";
    echo "<FORM METHOD=post NAME='menuform' ACTION='gratraficosfcl.php'>";
	...
	if (!$tm->Open("select ca_idtrafico, ca_nombre from vi_traficos order by ca_nombre")) {       // Selecciona todos lo registros de la tabla Traficos
	    ...
	    echo "<script>document.location.href = 'gratraficosfcl.php';</script>";
	    exit; }
	$tm->MoveFirst();
...
require_once("menu.php");
	...
	echo "<TR>";
	echo "  <TD Class=invertir><iframe ID=consulta_tar src ='gratraficosfcl.php?accion=Imprimir&ano=$ano&mes=$mes&traorigen=$traorigen' width='100%' height='100%'></iframe></TD>";
	echo "</TR>";
	echo "</TABLE>";
	echo "<TABLE CELLSPACING=10>";
	echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='accion' VALUE='Regresar' ONCLICK='javascript:document.location.href = \"gratraficosfcl.php\"'></TH>";  // Cancela la operación
	echo "</TABLE>";
	echo "</CENTER>";
    }
elseif (!isset($boton) and isset($accion) and isset($traorigen)){
