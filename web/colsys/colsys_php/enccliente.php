...
/*================-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*\
*/
$titulo = 'Maestra de Encuestas por Cliente';
$siono = array("Sí","No");
$tipos = array("Local","Oficina","Bodega","Apartamento","Casa","Piso","Planta de Producción");
$vigilancias = array("No Tiene", "Escoltas", "Privada");
$acondiciones = array("Normal","Rutinas","Lujoso");
...
$estados = array("Activo","Potencial","Perdido","Seguimiento");
...
//  include_once 'include/seguridad.php';                                      // Control de Acceso al módulo
    ...
    echo "<HTML>";
    echo "<HEAD>";
    echo "<TITLE>Tabla de Encuentas por Cliente</TITLE>";
    ...
    while (!$rs->Eof() and !$rs->IsEmpty()) {                                                      // Lee la totalidad de los registros obtenidos en la instrucción Select
	   ...
	   while($id_temp == $rs->Value('ca_idcliente') and !$rs->Eof()){
		  if ($rs->Value('ca_idencuesta') != '') {
	         echo "<TR>";
			 echo "  <TD Class=mostrar ROWSPAN=20>&nbsp&nbsp</TD>";
			 ...
			 echo "  <TD Class=listar ROWSPAN=20 onclick='elegir(\"Imprimir\", \"".$rs->Value('ca_idencuesta')."\", \"".$rs->Value('ca_idcliente')."\");'><IMG src='./graficos/pdf.gif' alt='Genera archivo PFD del Reporte de Visita' border=0></TD>";
	         echo "</TR>";
	         echo "<TR>";
			 ...
			 echo "  <TD Class=mostrar>¿Cuenta con un Sistema de Alarma? :</TD>";
             echo "  <TD Class=mostrar><B>".$rs->Value('ca_alarma')."</B></TD>";
	         echo "</TR>";
	         echo "<TR>";
	         ...
	         echo "</TR>";
	         echo "<TR>";
			 echo "  <TD Class=mostrar>¿El personal se ve bien presentado? :</TD>";
             echo "  <TD Class=mostrar><B>".$rs->Value('ca_perpresentado')."</B></TD>";
			 echo "  <TD Class=mostrar>¿Está el personal Uniformado? :</TD>";
             echo "  <TD Class=mostrar><B>".$rs->Value('ca_peruniformado')."</B></TD>";
	         echo "</TR>";
			 ...
			 echo "  <TD Class=mostrar>¿Es éste movimiento organizado? :</TD>";
             echo "  <TD Class=mostrar><B>".$rs->Value('ca_merorganizado')."</B></TD>";
	         echo "</TR>";
	         echo "<TR>";
			 echo "  <TD Class=mostrar>¿Hay existencias de Mercancia? :</TD>";
             echo "  <TD Class=mostrar><B>".$rs->Value('ca_merexistencias')."</B></TD>";
			 echo "  <TD Class=mostrar>¿Tiene Control de Acceso? :</TD>";
             echo "  <TD Class=mostrar><B>".$rs->Value('ca_mercontrol')."</B></TD>";
	         echo "</TR>";
             echo "<TR>";
             echo "  <TD Class=listar ROWSPAN=3>¿La bodega cuenta con infraestructura? :</TD>";
             echo "  <TD Class=listar ROWSPAN=3><B>".str_replace ("|","<BR>",$rs->Value('ca_merinfraestructura'))."</B></TD>";
             echo "  <TD Class=listar>¿Se supervisa el cargue? :</TD>";
             echo "  <TD Class=mostrar><B>".$rs->Value('ca_mersupervision')."</B></TD>";
             echo "</TR>";
             echo "<TR>";
             ...
             echo "</TR>";
             echo "<TR>";
             echo "  <TD Class=listar>¿Cargue supervisado con seguridad? :</TD>";
             echo "  <TD Class=mostrar><B>".$rs->Value('ca_merseguridad')."</B></TD>";
             echo "</TR>";
			 ...
			 echo "  <TD Class=mostrar>¿Refleja operar bajo la Legalida? :</TD>";
             echo "  <TD Class=mostrar><B>".$rs->Value('ca_legalidad')."</B></TD>";
	         echo "</TR>";
	         echo "<TR>";
			 echo "  <TD Class=mostrar>¿Ofrece algún peligro para Coltrans? :</TD>";
...
require_once("menu.php");
             ...
             echo "  </SELECT></TD>";
			 echo "  <TD Class=mostrar>¿Cuenta con un Sistema de Alarma? :</TD>";
             echo "  <TD Class=mostrar><SELECT NAME='alarma'>";
			 echo "      <OPTION VALUE=''></OPTION>";
             for ($i=0; $i < count($siono); $i++) {
                  echo " <OPTION VALUE='".$siono[$i]."'>".$siono[$i];
                  }
	         ...
	         echo "<TR>";
			 echo "  <TD Class=mostrar>¿El personal se ve bien presentado? :</TD>";
             echo "  <TD Class=mostrar><SELECT NAME='perpresentado'>";
			 echo "      <OPTION VALUE=''></OPTION>";
             for ($i=0; $i < count($siono); $i++) {
                  echo " <OPTION VALUE='".$siono[$i]."'>".$siono[$i];
                  }
             echo "  </SELECT></TD>";
			 echo "  <TD Class=mostrar>¿Está el personal Uniformado? :</TD>";
             echo "  <TD Class=mostrar><SELECT NAME='peruniformado'>";
			 echo "      <OPTION VALUE=''></OPTION>";
             for ($i=0; $i < count($siono); $i++) {
                  echo " <OPTION VALUE='".$siono[$i]."'>".$siono[$i];
                  }
             ...
             echo "  </SELECT></TD>";
			 echo "  <TD Class=mostrar>¿Es éste movimiento organizado? :</TD>";
             echo "  <TD Class=mostrar><SELECT NAME='merorganizado'>";
			 echo "      <OPTION VALUE=''></OPTION>";
             for ($i=0; $i < count($siono); $i++) {
                  echo " <OPTION VALUE='".$siono[$i]."'>".$siono[$i];
                  }
	         ...
	         echo "<TR>";
			 echo "  <TD Class=mostrar>¿Hay existencias de Mercancia? :</TD>";
             echo "  <TD Class=mostrar><SELECT NAME='merexistencias'>";
			 echo "      <OPTION VALUE=''></OPTION>";
             for ($i=0; $i < count($siono); $i++) {
                  echo " <OPTION VALUE='".$siono[$i]."'>".$siono[$i];
                  }
             ...
             echo "<TR>";
             echo "  <TD Class=listar ROWSPAN=3>¿La bodega cuenta con infraestructura? :</TD>";
             echo "  <TD Class=mostrar ROWSPAN=3>";
             for ($i=0; $i < count($infraestructuras); $i++) {
                  echo " <INPUT TYPE=CHECKBOX NAME='merinfraestructura[]' VALUE='".$infraestructuras[$i]."'>".$infraestructuras[$i]."<BR>";
                  }
             echo "  </TD>";
             echo "  <TD Class=listar>¿Se supervisa el cargue? :</TD>";
             echo "  <TD Class=mostrar><SELECT NAME='mersupervision'>";
			 echo "      <OPTION VALUE=''></OPTION>";
             for ($i=0; $i < count($siono); $i++) {
                  echo " <OPTION VALUE='".$siono[$i]."'>".$siono[$i];
                  }
             ...
             echo "<TR>";
             echo "  <TD Class=listar>¿Cargue supervisado con seguridad? :</TD>";
             echo "  <TD Class=mostrar><SELECT NAME='merseguridad'>";
			 echo "      <OPTION VALUE=''></OPTION>";
             for ($i=0; $i < count($siono); $i++) {
                  echo " <OPTION VALUE='".$siono[$i]."'>".$siono[$i];
                  }
    ...
    switch(trim($accion)) {                                                    // Switch que evalua cual botòn de comando fue pulsado por el usuario
        case 'Guardar': {                                                      // El Botón Guardar fue pulsado
             ...
             if (!$rs->Open("insert into tb_enccliente (ca_idcliente, ca_fchvisita, ca_idcontacto, ca_instalaciones, ca_compartidas, ca_condiciones, ca_vivienda, ca_vigilancia, ca_alarma, ca_masseguridad, ca_detseguridad, ca_peracorde, ca_percarne, ca_perpresentado, ca_peruniformado,
							 ...
							 values($id, '$fchvisita', $idcontacto, '$instalaciones', '$compartidas', '$condiciones', '$vivienda', '$vigilancia', '$alarma', '$masseguridad', '$detseguridad', '$peracorde', '$percarne', '$perpresentado', '$peruniformado', '$mermovimiento', '$merorganizado', '$merexistencias',
							 '$mercontrol', '$merinfraestructura','$mersupervision', '$mercargue', '$merseguridad', '$recomendable','$legalidad', '$peligro','$explicacion', '$actividad', '$estado', to_timestamp('".date("d M Y H:i:s")."', 'DD Mon YYYY hh:mi:ss'), '$usuario')")) {
                 echo "<script>alert(\"".addslashes($rs->mErrMsg)."\");</script>";  // Muestra el mensaje de error
