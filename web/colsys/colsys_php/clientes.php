...
//  include_once 'include/seguridad.php';                                      // Control de Acceso al módulo
    ...
    if ($salida[0] == "Columnas") {
        ...
        while (!$rs->Eof() and ! $rs->IsEmpty()) {                                                      // Lee la totalidad de los registros obtenidos en la instrucción Select
            ...
            echo "      <TR><TD style='visibility: $vista_2; text-align: center; color=blue;' Class=mostrar onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='javascript:document.location.href = \"/clientes/mandatosyPoderesExt4/id/" . $rs->Value('ca_idcliente') . "\"'><BR><IMG src='graficos/mandato.gif'><BR>Control<br>Mandatos</TD></TR>";
            ...
            $q = "SELECT mc.ca_idciudad, ci.ca_ciudad, mc.ca_idtipo, mt.ca_tipo, mt.ca_clase, mc.ca_fchradicado, mc.ca_fchvencimiento, mc.ca_observaciones"
                    . " FROM tb_mancliente mc"
                    . "     INNER JOIN tb_ciudades ci ON ci.ca_idciudad = mc.ca_idciudad"
                    . "     INNER JOIN tb_mandatos_tipo mt ON mt.ca_idtipo = mc.ca_idtipo"
                    . " WHERE mc.ca_idcliente = " . $rs->Value('ca_idcliente')
                    . " ORDER BY mt.ca_clase, mt.ca_tipo";
            ...
            if (!$tm->IsEmpty()) {
                ...
                echo "  <TD Class=listar COLSPAN=4 style='background-color: $col_mem;'><TABLE WIDTH=100% CELLSPACING=1 BORDER=1>";
                echo "  <TR>";
                echo "   <TH style='background-color: $col_mem;'>Ciudad</TH>";
                echo "   <TH style='background-color: $col_mem;'>Tipo</TH>";
                echo "   <TH style='background-color: $col_mem;'>Clase</TH>";
                echo "   <TH style='background-color: $col_mem;'>Radicado</TH>";
                echo "   <TH style='background-color: $col_mem;'>Vencimiento</TH>";
                echo "   <TH style='background-color: $col_mem;'>Observaciones</TH>";
                echo "  </TR>";
                $tit_mem = true;
            }
            $tm->MoveFirst();
            while (!$tm->Eof() and ! $tm->IsEmpty()) {
                ...
                echo "    <TD Class=listar style='background-color: $col_mem;'>" . $tm->Value('ca_clase') . "</TD>";
                ...
                echo "    <TD Class=listar style='background-color: $col_mem;'>" . $tm->Value('ca_observaciones') . "</TD>";
                echo "  </TR>";
                $tm->MoveNext();
            }
            if ($tit_mem) {
