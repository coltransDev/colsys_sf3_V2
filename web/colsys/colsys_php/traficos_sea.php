<?php
/*================-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*\
// Archivo:       TRAFICOS_SEA.PHP                                            \\
// Creado:        2005-04-20                                                  \\
// Autor:         Carlos Gilberto López M.                                    \\
// Ver:           1.00                                                        \\
// Updated:       2005-06-01                                                  \\
//                                                                            \\
// Descripción:   Módulo para el control de Negocios Marítimos.               \\
//                                                                            \\
//                                                                            \\
...
if (!isset($criterio) and !isset($boton) and !isset($accion)){
    ...
    echo "  document.location.href = 'traficos_sea.php?boton='+opcion+'\&id='+id+'\&cl='+cl;";
    echo "}";
...
require_once("menu.php");
    ...
    echo "<CENTER>";
    echo "<H3>$titulo</H3>";
    echo "<FORM METHOD=post NAME='menuform' ACTION='traficos_sea.php' >";
...
//  include_once 'include/seguridad.php';                                      // Control de Acceso al módulo
    ...
    if (!$rs->Open("select * from vi_reportes $condicion ca_transporte = 'Marítimo'")) {                       // Selecciona todos lo registros de la tabla Ino-Marítimo
...
require_once("menu.php");
    echo "<STYLE>@import URL(\"Coltrans.css\");</STYLE>";                      // Carga una hoja de estilo que estandariza las pantallas den sistema graficador
    echo "<CENTER>";
    echo "<FORM METHOD=post NAME='cabecera' ACTION='traficos_sea.php'>";             // Hace una llamado nuevamente a este script pero con
    ...
    while (!$rs->Eof() and !$rs->IsEmpty()) {                                                      // Lee la totalidad de los registros obtenidos en la instrucción Select
       ...
       echo "  <TD Class=listar ROWSPAN=2 onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='javascript:document.location.href=\"traficos_sea.php?boton=Consultar\&id=".$rs->Value('ca_idreporte')."\";'>No.&nbsp;".$rs->Value('ca_version')."</TD>";
    ...
    echo "</TABLE><BR>";
    echo "<TABLE CELLSPACING=10>";
    echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='boton' VALUE='Regresar' ONCLICK='javascript:document.location.href = \"traficos_sea.php\"'></TH>";  // Cancela la operación
    echo "</TABLE>";
    echo "</FORM>";
    echo "</CENTER>";
//  echo "<P DIR='RTL'><A HREF=\"#\" ONCLICK='javascript:window.open(\"./help/$modulo.html\",\"Ayuda\",\"scrollbars=yes,width=600,height=400,top=200,left=150\")'><IMG SRC='./graficos/help.gif' border=0 ALT='Ayuda en Línea'><BR>Ayuda</A></P>";  // Link que proporciona la Ayuda en línea
...
elseif (isset($boton)) {                                                       // Switch que evalua cual botòn de comando fue pulsado por el usuario
    switch(trim($boton)) {
        case 'Consultar': {                                                    // Opcion para Consultar un solo registro
             ...
             echo "    document.location.href = 'traficos_sea.php?boton='+opcion+'\&id='+id;";
             echo "}";
             echo "</script>";
             echo "</HEAD>";
             echo "<BODY>";
require_once("menu.php");
             ...
             echo "<CENTER>";
             echo "<H3>$titulo</H3>";
             echo "<FORM METHOD=post NAME='adicionar' ACTION='traficos_sea.php' ONSUBMIT='return validar();'>";// Crea una forma con datos vacios
             datos_basicos($rs,$tm);
             ...
             echo "<TR>";
             echo "  <TH Class=titulo COLSPAN=6>Reporte Marítimo al Exterior</TH>";
             echo "</TR>";
...
require_once("menu.php");
             ...
             echo "</TABLE>";
             echo "<TABLE CELLSPACING=10>";
             echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='accion' VALUE='Regresar' ONCLICK='javascript:document.location.href = \"traficos_sea.php?boton=Consultar&id=$id\"'></TH>";  // Cancela la operación
             echo "</TABLE>";
             echo "</CENTER>";
             break;
             }
...
//           include_once 'include/seguridad.php';                         // Control de Acceso al módulo
             ...
             $contenido.="<TR>\n";
             $contenido.="  <TD COLSPAN=4><CENTER><B>SEAFREIGHT BUSINESS REPORT</B></CENTER></TD>\n";
             $contenido.="</TR>\n";
             ...
             while (!$tm->Eof()) {
                 $contenido.="<TR>\n";
                 $contenido.="  <TD WIDTH=24%><B>SHIPPER ".(($tm->GetRowCount()>1)?"# ".$tm->GetCurrentRow():"").":</B></TD>\n";
                 $contenido.="  <TD WIDTH=76% COLSPAN=3>".$tm->Value('ca_nombre')."</TD>\n";
                 $contenido.="</TR>\n";
                 $contenido.="<TR>\n";
                 ...
                 $contenido.="</TR>\n";
                 $contenido.="<TR>\n";
                 $contenido.="  <TD><B>ORDER SUPPLIER :</B></TD>\n";
             ...
             $contenido.="<TR>\n";
             $contenido.="  <TD><B>ORDER CONSIGNEE :</B></TD>\n";
			 ...
			 if ($rs->Value('ca_idmaster') != 0){
	             $consignatario_m = $rs->Value('ca_nombre_mas')."<BR>".$rs->Value('ca_contacto_mas')."<BR>Dirección: ".$rs->Value('ca_direccion_mas')."<BR>Teléfonos:".$rs->Value('ca_telefonos_mas')." Fax:".$rs->Value('ca_fax_mas')."<BR>Email: ".$rs->Value('ca_email_mas');
			 }else {
				 if (!$tm->Open("select cl.ca_idcliente, cl.ca_digito, cl.ca_compania, sc.ca_direccion, sc.ca_telefono, sc.ca_fax, sc.ca_nombre, cl.ca_pais from vi_clientes cl LEFT OUTER JOIN control.tb_sucursales sc ON (sc.ca_nombre = 'Bogotá D.C.') where ca_compania = 'COLTRANS S.A.'")) { // Selecciona todos los Datos de Coltrans / Bogotá
					 echo "<script>alert(\"".addslashes($tm->mErrMsg)."\");</script>";                   // Muestra esl mensaje de error
					 echo "<script>document.location.href = 'entrada.php';</script>";
					 exit; }
				 $consignatario_m = $tm->Value('ca_compania')." Nit. ".number_format($tm->Value('ca_idcliente'),0)."-".$tm->Value('ca_digito')."<BR>Dirección: ".$tm->Value('ca_direccion')."<BR>Teléfonos:".$tm->Value('ca_telefono')." Fax:".$tm->Value('ca_fax')."<BR>".$tm->Value('ca_nombre')." - ".$tm->Value('ca_pais');
			 }

			 $consignatario_final = ($rs->Value('ca_idconsignatario')!=0)?$rs->Value('ca_nombre_con')." Nit. ".$rs->Value('ca_identificacion_con')."<BR>Contacto: ".$rs->Value('ca_contacto_con')."<BR>Dirección: ".$rs->Value('ca_direccion_con')."<BR>Teléfonos:".$rs->Value('ca_telefonos_con')." Fax:".$rs->Value('ca_fax_con')."<BR>Email: ".$rs->Value('ca_email_con'):$rs->Value('ca_nombre_cli')." Nit. ".number_format($rs->Value('ca_idcliente'),0)."-".$rs->Value('ca_digito')."<BR>Contacto: ".$rs->Value('ca_contacto_cli')."<BR>Dirección: ".str_replace ("|"," ",$rs->Value('ca_direccion_cli'))."<BR>Teléfonos: ".$rs->Value('ca_telefonos_cli')." Fax: ".$rs->Value('ca_fax_cli')."<BR>Email: ".$rs->Value('ca_email_cli');
             $hijo = (($rs->Value('ca_idconsignar')==1)?$consignatario_final:$rs->Value('ca_consignar')).(($rs->Value('ca_tipobodega')!= "Coordinador Logistico")?" / ".$rs->Value('ca_tipobodega')." ".(($rs->Value('ca_bodega')!='N/A')?$rs->Value('ca_bodega'):"")." ".$rs->Value('ca_final_dest')." - ".$tm->Value('ca_pais'):"");

			 if ($rs->Value('ca_notify') == 0)
	             $notify_h = $rs->Value('ca_nombre_cli')." Nit. ".number_format($rs->Value('ca_idcliente'),0)."-".$rs->Value('ca_digito')."<BR>Contacto: ".$rs->Value('ca_contacto_cli')."<BR>Dirección: ".str_replace ("|"," ",$rs->Value('ca_direccion_cli'))."<BR>Teléfonos:".$rs->Value('ca_telefonos_cli')." Fax:".$rs->Value('ca_fax_cli')."<BR>Email: ".$rs->Value('ca_email_cli')."<BR>".$tm->Value('ca_pais');
			 else if ($rs->Value('ca_notify') == 1)
	             $notify_h = $rs->Value('ca_nombre_con')." Nit. ".$rs->Value('ca_identificacion_con')."<BR>Contacto: ".$rs->Value('ca_contacto_con')."<BR>Dirección: ".$rs->Value('ca_direccion_con')."<BR>Teléfonos:".$rs->Value('ca_telefonos_con')." Fax:".$rs->Value('ca_fax_con')."<BR>Email: ".$rs->Value('ca_email_con');
			 else if ($rs->Value('ca_notify') == 2)
	             $notify_h = $rs->Value('ca_nombre_not')."<BR>".$rs->Value('ca_contacto_not')."<BR>Dirección: ".$rs->Value('ca_direccion_not')."<BR>Contacto: ".$rs->Value('ca_contacto_not')."<BR>Teléfonos:".$rs->Value('ca_telefonos_not')." Fax:".$rs->Value('ca_fax_not')."<BR>Email: ".$rs->Value('ca_email_not');
			 else if ($rs->Value('ca_notify') == 3)
	             $notify_h = $rs->Value('ca_nombre_mas')."<BR>".$rs->Value('ca_contacto_mas')."<BR>Dirección: ".$rs->Value('ca_direccion_mas')."<BR>Contacto: ".$rs->Value('ca_contacto_mas')."<BR>Teléfonos:".$rs->Value('ca_telefonos_mas')." Fax:".$rs->Value('ca_fax_mas')."<BR>Email: ".$rs->Value('ca_email_mas');
             ...
             if ($rs->Value('ca_idlinea') != 0){
                $contenido.="<TR>\n";
                $contenido.="  <TD STYLE=\"vertical-align:top\"><B>SHIPPING LINE:</B></TD>\n";
                $contenido.="  <TD COLSPAN=3>".$rs->Value('ca_nombre')."</TD>\n";
                $contenido.="</TR>\n";
             }
             if ($rs->Value('ca_modalidad') != 'LCL'){
                $contenido.="<TR>\n";
                $contenido.="  <TD STYLE=\"vertical-align:top\"><B>MBL CONSIGNED TO:</B></TD>\n";
                $contenido.="  <TD COLSPAN=3>$master</TD>\n";
                $contenido.="</TR>\n";
                $contenido.="<TR>\n";
                $contenido.="  <TD STYLE=\"vertical-align:top\"><B>NOTIFY:</B></TD>\n";
                $contenido.="  <TD COLSPAN=3>$consignatario_m</TD>\n";
                $contenido.="</TR>\n";
             }
             $contenido.="<TR>\n";
             $contenido.="  <TD COLSPAN=4>&nbsp;</TD>\n";
             $contenido.="</TR>\n";
             $contenido.="<TR>\n";
             $contenido.="  <TD STYLE=\"vertical-align:top\"><B>HBL CONSIGNED TO:</B></TD>\n";
             $contenido.="  <TD COLSPAN=3>$hijo</TD>\n";
             $contenido.="</TR>\n";
             $contenido.="<TR>\n";
             $contenido.="  <TD STYLE=\"vertical-align:top\"><B>NOTIFY:</B></TD>\n";
             $contenido.="  <TD COLSPAN=3>$notify_h</TD>\n";
             $contenido.="</TR>\n";
             ...
             while (!$tm->Eof() and !$tm->IsEmpty()) {
                 $con_mem = "<B>Ocean Freight :</B><BR>".$tm->Value('ca_concepto')."";
				 $con_mem.= (($tm->Value('ca_cantidad')>0)?" [Cant.: ".formatNumber($tm->Value('ca_cantidad'),3)."]":"");

                 $sub_conte ="<TABLE BORDER=1 CELLSPACING=0 WIDTH=100%>\n";
				 $sub_conte.="<TR>\n";
                 if ($tm->Value('ca_neta_tar') > 0){
                    $sub_conte.="<TD BGCOLOR=\"#CCCCCC\" WIDTH=25%><B>Buying Rate :</B><BR>".number_format($tm->Value('ca_neta_tar'),2)." ".$tm->Value('ca_neta_idm')."</TD>\n";
                 }
                 if ($tm->Value('ca_neta_min') > 0){
                    $sub_conte.="<TD BGCOLOR=\"#CCCCCC\" WIDTH=25%><B>Minimum :</B><BR>".number_format($tm->Value('ca_neta_min'),2)." ".$tm->Value('ca_neta_idm')."</TD>\n";
                 }
                 if ($tm->Value('ca_reportar_tar') > 0){
                    $sub_conte.="<TD BGCOLOR=\"#CCCCCC\" WIDTH=25%><B>Selling Rate :</B><BR>".number_format($tm->Value('ca_reportar_tar'),2)." ".$tm->Value('ca_reportar_idm')."</TD>\n";
                 }
                 if ($tm->Value('ca_reportar_min') > 0){
                    $sub_conte.="<TD BGCOLOR=\"#CCCCCC\" WIDTH=25%><B>Minimum :</B><BR>".number_format($tm->Value('ca_reportar_min'),2)." ".$tm->Value('ca_reportar_idm')."</TD>\n";
                 }
/*
...
*/
                 ...
                 $contenido.="<TR>\n";
                 $contenido.="  <TD STYLE=\"vertical-align:bottom\" BGCOLOR=\"#CCCCCC\" WIDTH=30%>$con_mem</TD>\n";
                 ...
                 while (!$gs->Eof() and !$gs->IsEmpty()) {
                    if ($gs->Value('ca_idconcepto') == $tm->Value('ca_idconcepto')){
                        $sub_conte ="<TABLE BORDER=1 CELLSPACING=0 WIDTH=100%>\n";
                        $sub_conte.="<TR>\n";
                        if ($gs->Value('ca_neta_tar') != 0){
                            $sub_conte.="  <TD><B>Buying Rate :</B><BR>".(($gs->Value('ca_tipo')=='$')?number_format($gs->Value('ca_neta_tar'),2)." ".$gs->Value('ca_idmoneda'):number_format($gs->Value('ca_neta_tar'),2).$gs->Value('ca_tipo'))."</TD>\n";
                        }
                        if ($gs->Value('ca_neta_min') != 0){
                            $sub_conte.="  <TD><B>Minimum :</B><BR>".number_format($gs->Value('ca_neta_min'),2)." ".$gs->Value('ca_idmoneda')."</TD>\n";
                        }
                        if ($gs->Value('ca_reportar_tar') != 0){
                            $sub_conte.="  <TD><B>Selling Rate :</B><BR>".(($gs->Value('ca_tipo')=='$')?number_format($gs->Value('ca_reportar_tar'),2)." ".$gs->Value('ca_idmoneda'):number_format($gs->Value('ca_reportar_tar'),2).$gs->Value('ca_tipo'))."</TD>\n";
                        }
                        if ($gs->Value('ca_reportar_min') != 0){
                            $sub_conte.="  <TD><B>Minimum :</B><BR>".number_format($gs->Value('ca_reportar_min'),2)." ".$gs->Value('ca_idmoneda')."</TD>\n";
                        }
                        $sub_conte.="</TR>\n";
                        $sub_conte.="</TABLE>\n";

                        $contenido.="<TR>\n";
                        $contenido.="  <TD STYLE=\"vertical-align:bottom\" WIDTH=30%><B>".$gs->Value('ca_recargo')."</B></TD>\n";
                        $contenido.="  <TD STYLE=\"vertical-align:bottom\" WIDTH=70%>$sub_conte</TD>\n";
                        $contenido.="</TR>\n";
                    }
                    $gs->MoveNext();
                 }
             ...
             $contenido.="<TR>\n";
             $contenido.="  <TD BGCOLOR=\"#CCCCCC\" COLSPAN=4><CENTER><B>General Charges</B></CENTER></TD>\n";
             $contenido.="</TR>\n";
             $gs->MoveFirst();
             while (!$gs->Eof() and !$gs->IsEmpty()) {
             	if ($gs->Value('ca_neta_tar') == 0 and $gs->Value('ca_reportar_tar') == 0) {
             	    $gs->MoveNext();
             		continue;
             	}
                if ($gs->Value('ca_idconcepto') == '9999'){
					$sub_conte ="<TABLE BORDER=1 CELLSPACING=0 WIDTH=100%>\n";
					$sub_conte.="<TR>\n";
                    if ($gs->Value('ca_neta_tar') != 0){
                        $sub_conte.="  <TD><B>Buying Rate :</B><BR>".(($gs->Value('ca_tipo')=='$')?number_format($gs->Value('ca_neta_tar'),2)." ".$gs->Value('ca_idmoneda'):number_format($gs->Value('ca_neta_tar'),2).$gs->Value('ca_tipo'))."</TD>\n";
             ...
             echo "</DIV>";
             echo "<STYLE>@import URL(\"Coltrans.css\");</STYLE>";             // Carga una hoja de estilo que estandariza las pantallas den sistema graficador
             echo "<FORM METHOD=post NAME='email' ACTION='traficos_sea.php' ONSUBMIT='return validar();' enctype='multipart/form-data'>";
             ...
             echo "</TR>";
             if (!$tm->Open("select ca_nombre, ca_email from vi_contactos where ca_idagente = '".$rs->Value('ca_idagente')."' and ca_impoexpo like '%".(($rs->Value('ca_impoexpo')=='Triangulación')?'Importación':$rs->Value('ca_impoexpo'))."%' and ca_transporte like '%".$rs->Value('ca_transporte')."%'")) { // Selecciona todos lo registros de la tabla Agentes
                 echo "<script>alert(\"".addslashes($tm->mErrMsg)."\");</script>";                   // Muestra el mensaje de error
                 echo "<script>document.location.href = 'entrada.php';</script>";
                 exit; }
             echo "<TR>";
...
*/
             ...
             echo "</TR>";
             echo "<TR>";
             echo "  <TD Class=listar COLSPAN=2>Instrucciones Especiales para el Agente:&nbsp;<IMG src='graficos/cerrado.gif' ALT='Click para incluir textos predefinidos' ONCLICK='terceros(\"find_texts\",\"Instructions_sea\",\"ventanas\");'><BR><TEXTAREA ID=instrucciones NAME='mensaje[]' WRAP=virtual ROWS=5 COLS=88>".$rs->Value('ca_instrucciones')."</TEXTAREA></TD>";
             echo "</TR>";
             echo "<TR>";
             ...
             echo "</TR>";
             echo "<TH Class=titulo><INPUT TYPE='BUTTON' NAME='accion' VALUE='Cancelar ' ONCLICK='javascript:document.location.href = \"traficos_sea.php?boton=Consultar\&id=$id\"'></TH>";
...
elseif (isset($accion)) {                                                       // Switch que evalua cual botòn de comando fue pulsado por el usuario
     ...
     switch(trim($accion)) {                                                    // Switch que evalua cual botòn de comando fue pulsado por el usuario
        case 'Enviar Mensaje': {                                                // El Botón Enviar Mensaje fue pulsado
            ...
            /*if (!$rs->Open("update tb_reportes set ca_idetapa = 'IMCAG', ca_fchultstatus=CURRENT_TIMESTAMP where ca_idreporte = $id")) {
                echo "<script>alert(\"".addslashes($rs->mErrMsg)."\");</script>";  // Muestra el mensaje de error
     ...
     if (isset($id)) {
        echo "<script>document.location.href = 'traficos_sea.php?boton=Consultar\&id=$id';</script>";  // Retorna a la pantalla principal de la opción
     }else {
        echo "<script>document.location.href = 'traficos_sea.php';</script>";  // Retorna a la pantalla principal de la opción
     }
   }
