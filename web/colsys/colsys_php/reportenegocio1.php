...
if (!isset($criterio) and !isset($boton) and !isset($accion)) {
    ...
    echo "    document.location.href = 'reportenegocio1.php?boton='+opcion+'\&id='+id+'\&cl='+cl;";
    echo "}";
    ...
    echo "}";
    echo "</script>";
    echo "<script language='javascript' src='javascripts/popcalendar1.js'></script>";
    echo "</HEAD>";
    echo "<BODY>";
    ...
    echo "<CENTER>";
    echo "<H3>$titulo</H3>";
    echo "<FORM METHOD=post NAME='menuform' ACTION='reportenegocio1.php' ONSUBMIT='return validar();'>";
    ...
    $vista_1 = ($nivel >= 0)?'block':'none'; 					// Habilita la opción para creación de reportes AG
    echo "<TH>";
    echo "   <TABLE CELLSPACING=1>";
    echo "      <TR>";
    echo "          <TD><IMG src='./graficos/new_ag.gif' alt='Crear Reporte AG' border=0 onclick='elegir(\"Reporte_Ag\", 0, 0);' style='display: $vista_1;'></TH></TD>";  // Botón para la creación de un Registro Nuevo
    ...
    while (!$rs->Eof() and !$rs->IsEmpty()) {                                                      // Lee la totalidad de los registros obtenidos en la instrucción Select
        ...
        echo "<TR>";
        echo "  <TD style='font-size: 9px;' Class=listar ROWSPAN=2 onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick=\"javascript:location.href = 'reportenegocio1.php?boton=Consultar&id=".$rs->Value('ca_idreporte')."'\">".$rs->Value('ca_consecutivo')."</TD>";
...
elseif (!isset($boton) and !isset($accion) and isset($criterio)) {
    ...
    echo "<STYLE>@import URL(\"Coltrans.css\");</STYLE>";                      // Carga una hoja de estilo que estandariza las pantallas den sistema graficador
    echo "<CENTER>";
    echo "<FORM METHOD=post NAME='cabecera' ACTION='reportenegocio1.php'>";             // Hace una llamado nuevamente a este script pero con
    ...
    if ($opcion != 'ca_contenido') {
        ...
        while (!$rs->Eof() and !$rs->IsEmpty()) {                                                      // Lee la totalidad de los registros obtenidos en la instrucción Select
            ...
            if ($consecutivo <> $rs->Value('ca_consecutivo')) {
                ...
                if (!$tm->Open("select * from vi_terceros where $cadena")) {
                    ...
                    echo "<script>document.location.href = 'reportenegocio1.php';</script>";
                    exit;
                }
                $tm->MoveFirst();
            ...
            echo "  <TD Class=listar ROWSPAN=2 onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='javascript:document.location.href=\"reportenegocio1.php?boton=Consultar\&id=".$rs->Value('ca_idreporte')."\";'>No.&nbsp;".$rs->Value('ca_version')."</TD>";
        ...
        while (!$rs->Eof() and !$rs->IsEmpty()) {                                                      // Lee la totalidad de los registros obtenidos en la instrucción Select
            ...
            echo "  <TD Class=listar COLSPAN=2 onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='javascript:document.location.href=\"reportenegocio1.php?boton=Adicionar\&br=".$rs->Value('oid')."\";'>".$rs->Value('ca_fchcreado')."</TD>";
    ...
    echo "</TABLE><BR>";
    echo "<TABLE CELLSPACING=10>";
    echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='boton' VALUE='Regresar' ONCLICK='javascript:document.location.href = \"reportenegocio1.php\"'></TH>";  // Cancela la operación
    echo "</TABLE>";
    echo "</FORM>";
    echo "</CENTER>";
    //    echo "<P DIR='RTL'><A HREF=\"#\" ONCLICK='javascript:window.open(\"./help/$modulo.html\",\"Ayuda\",\"scrollbars=yes,width=600,height=400,top=200,left=150\")'><IMG SRC='./graficos/help.gif' border=0 ALT='Ayuda en Línea'><BR>Ayuda</A></P>";  // Link que proporciona la Ayuda en línea
...
elseif (isset($boton)) {                                                       // Switch que evalua cual botòn de comando fue pulsado por el usuario
    switch(trim($boton)) {
        case 'Consultar': {                                                    // Opcion para Consultar un solo registro
                ...
                $visible = ($rs->Value('ca_login')== $usuario or $nivel >= 1)?'block':'none';
                ...
                echo "            document.location.href = 'reportenegocio1.php?accion='+opcion+'\&id='+id;";
                echo "        }";
                ...
                echo "                     document.location.href = 'reportenegocio1.php?accion='+opcion+'\&id='+id+'\&det='+det_mem;";
                echo "            }";
                echo "        }";
                echo "    }else{";
                echo "        document.location.href = 'reportenegocio1.php?boton='+opcion+'\&id='+id;";
                echo "    }";
                echo "}";
                echo "</script>";
                echo "</HEAD>";
                ...
                echo "<CENTER>";
                echo "<H3>$titulo</H3>";
                echo "<FORM METHOD=post NAME='adicionar' id='adicionar' ACTION='reportenegocio1.php' ONSUBMIT='return validar();'>";// Crea una forma con datos vacios
                ...
                echo "        <TD Class=invertir style='text-align:center;' WIDTH='2%'><IMG style='display: $visible;' src='./graficos/edit.gif' alt='Editar el Registro' border=0 onclick='elegir(\"Liquidar\", $id);'>";
                echo "      </TR>";
                echo "  </TABLE></TD>";
                echo "</TR>";
        ...
        case 'Adicionar': {                                                    // Opcion para Adicionar Registros a la tabla
                ...
                echo "<HEAD>";
                echo "<TITLE>$titulo</TITLE>";
                echo "<script language='javascript' src='../js/jquery/jquery1.4-2.min.js'></script>";
                echo "<script language='javascript' src='javascripts/valreporte1.js'></script>";
                echo "<script language='javascript' src='javascripts/popcalendar1.js'></script>";
                echo "</HEAD>";

                echo "<BODY ID=Cuerpo onscroll='dalt=document.body.scrollTop+3; find_contacto.style.top=dalt; find_texts.style.top=dalt'>";
                require_once("menu.php");
                echo "<DIV ID='find_contacto' STYLE='display:none; position:absolute; border-width:3; border-color:#666666; border-style:solid;'>";
                ...
                echo "</IFRAME>";
                echo "</DIV>";
                echo "<DIV ID='find_texts' STYLE='display:none; position:absolute; border-width:3; border-color:#666666; border-style:solid;'>";
                ...
                echo "<CENTER>";
                echo "<H3>$titulo</H3>";
                echo "<FORM METHOD=post NAME='adicionar' id='adicionar' ACTION='reportenegocio1.php' ONSUBMIT='return validador();'>";// Crea una forma con datos vacios
                ...
                echo "  <TD Class=mostrar COLSPAN=2 style='text-align:left; vertical-align:bottom;'>Fecha del Reporte: <INPUT TYPE='TEXT' NAME='fchreporte' id='fchreporte' SIZE=12 VALUE='".date("Y-m-d")."' READONLY></TD>";
                ...
                echo "  </SELECT>&nbsp;&nbsp;&nbsp;</TD>";
                echo "  <TD Class=listar WIDTH=160><SELECT NAME='idtraorigen' id='idtraorigen' ONCHANGE='llenar_origenes();' ONMOUSEWHEEL='return false;'>";  // Llena el cuadro de lista con los valores de la tabla Traficos
                echo "  </SELECT></TD>";
                echo "  <TD Class=listar WIDTH=160><SELECT NAME='idciuorigen' id='idciuorigen' ONMOUSEWHEEL='return false;' SIZE=7>";          // Llena el cuadro de lista con los valores de la tabla Origenes
                echo "  </SELECT></TD>";
                echo "  <TD Class=listar WIDTH=160><SELECT NAME='idtradestino' id='idtradestino' ONCHANGE='llenar_destinos();' ONMOUSEWHEEL='return false;'>"; // Llena el cuadro de lista con los valores de la tabla Traficos
                echo "  </SELECT></TD>";
                echo "  <TD Class=listar WIDTH=160><SELECT NAME='idciudestino' id='idciudestino' ONMOUSEWHEEL='return false;' SIZE=7 ONCLICK='continuacion_dest.selectedIndex=idciudestino.selectedIndex'>";         // Llena el cuadro de lista con los valores de la tabla Destinos
                echo "  </SELECT></TD>";
                echo "</TR>";
                echo "<TR>";
                ...
                echo "  <TD Class=listar COLSPAN=4>5. Agente:&nbsp;&nbsp;[Listar Todos&nbsp;<INPUT TYPE='CHECKBOX'id='todos_ag' NAME='todos_ag' WIDTH=10 ONCHANGE='llenar_agentes();'>]<BR><SELECT NAME='idagente' id='idagente' ONMOUSEWHEEL='return false;'>";                              // Llena el cuadro de lista con los valores de la tabla Agentes
                echo "  </SELECT></TD>";
                echo "</TR>";
                echo "<TR>";
                ...
                echo "    <TD Class=mostrar style='text-align:right;' onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='terceros(\"find_contacto\",\"_pro_1\",\"findtercero1\");'><a><IMG src='graficos/lupa.gif' alt='Buscar' hspace='0' vspace='0'>&nbsp;Buscar</a></TD>";
                echo "  </TR>";
                echo "  <TR>";
                ...
                echo "    <TD Class=mostrar>8.1 Orden:<BR><INPUT TYPE='TEXT' NAME='orden_clie' id='orden_clie' SIZE=15 MAXLENGTH=255></TD>";
                echo "    <TD Class=mostrar style='text-align:right;' onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='terceros(\"find_contacto\",\"_cli\",\"findcontacto1\");'><a><IMG src='graficos/lupa.gif' alt='Buscar' hspace='0' vspace='0'> Buscar</a></TD>";
                echo "  </TR>";
                echo "  <TR>";
                ...
                echo "    <TD Class=mostrar style='text-align:right;' onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='terceros(\"find_contacto\",\"_con\",\"findtercero1\");'><a><IMG src='graficos/lupa.gif' alt='Buscar' hspace='0' vspace='0'> Buscar</a></TD>";
                echo "  </TR>";
                echo "  <TR>";
                ...
                echo "    <TD Class=mostrar style='text-align:right;' onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='terceros(\"find_contacto\",\"_not\",\"findtercero1\");'><a><IMG src='graficos/lupa.gif' alt='Buscar' hspace='0' vspace='0'> Buscar</a></TD>";
                echo "  </TR>";
                echo "  <TR>";
                ...
                echo "    <TD Class=mostrar style='text-align:right;' onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='terceros(\"find_contacto\",\"_mas\",\"findtercero1\");'><a><IMG src='graficos/lupa.gif' alt='Buscar' hspace='0' vspace='0'> Buscar</a></TD>";
                echo "  </TR>";
                echo "  <TR>";
                ...
                echo "    <TD Class=mostrar style='text-align:right;' onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='terceros(\"find_contacto\",\"_rep\",\"findtercero1\");'><a><IMG src='graficos/lupa.gif' alt='Buscar' hspace='0' vspace='0'> Buscar</a></TD>";
                echo "  </TR>";
                echo "  <TR>";
                ...
                for ($i=1; $i<=18; $i++) {
                    echo "  <TR>";
                    echo "    <TD Class=invertir style='vertical-align:bottom;' WIDTH=130><INPUT ID=conf_$z  TYPE='TEXT' NAME='contactos[]' VALUE=' ' ONCHANGE='cambiar_email(this);' SIZE=40 MAXLENGTH=50></TD><TD Class=invertir><INPUT ID=email_$z TYPE='CHECKBOX' NAME='confirmar[]' WIDTH=10 ONCHANGE='asignar_email(this);'></TD>";
                    echo "  </TR>";
                    $z++;
                }
                echo "  </TABLE></TD>";
                ...
                echo "      <TD Class=mostrar>13. Modalidad:<BR><SELECT name='modalidad' id='modalidad' ONCHANGE='llenar_lineas();'>";
                echo "      </SELECT></TD>";
                echo "      <TD Class=mostrar COLSPAN=3>14. Línea Transporte:<BR><SELECT NAME='idlinea' id='idlinea'>";             // Llena el cuadro de lista con los valores de la tabla Transportistas
                echo "      </SELECT></TD>";
                echo "    </TR>";
                echo "    <TR HEIGHT=5>";
                echo "      <TD Class=invertir COLSPAN=4></TD>";
                ...
                echo "      <TD Class=mostrar>Tiempo de Crédito:<BR><CENTER><INPUT DISABLED TYPE='TEXT' NAME='tiempocredito' id='tiempocredito' VALUE='-' SIZE=18 MAXLENGTH=20></CENTER></TD>";
                echo "    </TR>";
                echo "    <TR HEIGHT=5>";
                echo "      <TD Class=invertir COLSPAN=4></TD>";
                echo "    </TR>";
               ...
               echo "    <TR>";
                ...
                echo "      <TD Class=mostrar COLSPAN=3><SELECT NAME='tipo' id='tipo' ONCHANGE='llenar_bodegas();'></SELECT></TD>";
                echo "    </TR>";
                echo "    <TR>";
                ...
                echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='boton' VALUE='Cancelar' ONCLICK='javascript:document.location.href = \"reportenegocio1.php\"'></TH>";  // Cancela la operación
        ...
        case 'Reporte_Ag': {                                                    // Opcion para Adicionar Registros a la tabla
                ...
                echo "<CENTER>";
                echo "<H3>$titulo</H3>";
                echo "<FORM METHOD=post NAME='adicionar' id='adicionar' ACTION='reportenegocio1.php' ONSUBMIT='return validador();' enctype='multipart/form-data'>";// Crea una forma con datos vacios
                echo "<INPUT TYPE='HIDDEN' NAME='validado' VALUE='false'>";              // Hereda el Id del registro que se esta modificando
                ...
                echo "  <TD Class=listar WIDTH=160><SELECT NAME='idtradestino' id='idtradestino' ONCHANGE='llenar_destinos();' ONMOUSEWHEEL='return false;'>"; // Llena el cuadro de lista con los valores de la tabla Traficos
                echo "  </SELECT></TD>";
                echo "  <TD Class=listar WIDTH=160><SELECT NAME='idciudestino' id='idciudestino' ONMOUSEWHEEL='return false;' SIZE=7 ONCLICK='select_final();'>";         // Llena el cuadro de lista con los valores de la tabla Destinos
                echo "  </SELECT></TD>";
                echo "</TR>";
                echo "<TR>";
                echo "  <TD Class=titulo style='text-align:left; vertical-align:top;' ROWSPAN=2>4. Fecha Despacho:<BR><CENTER><INPUT TYPE='TEXT' NAME='fchdespacho' SIZE=12 VALUE='".date("Y-m-d")."' ONKEYDOWN=\"chkDate(this)\" ONDBLCLICK=\"popUpCalendar(this, this, 'yyyy-mm-dd')\"></CENTER></TD>";
                echo "  <TD Class=listar COLSPAN=4>5. Agente:&nbsp;&nbsp;[Listar Todos&nbsp;<INPUT TYPE='CHECKBOX' NAME='todos_ag' WIDTH=10 ONCHANGE='llenar_agentes();'>]<BR><SELECT NAME='idagente' id='idagente' ONMOUSEWHEEL='return false;'>";                              // Llena el cuadro de lista con los valores de la tabla Agentes
                echo "  </SELECT></TD>";
                echo "</TR>";
                echo "<TR>";
                echo "  <TD Class=mostrar COLSPAN=4>6. Descripción de la Mercancía:<BR><TEXTAREA NAME='mercancia_desc' WRAP=virtual ROWS=3 COLS=93></TEXTAREA><BR>¿Es Mercancía Peligrosa? <INPUT TYPE=CHECKBOX NAME='mcia_peligrosa'><IMG SRC='./graficos/nuevo.gif' border=0 ALT='Marque esta opción para indicar que Sí es Mercancia Peligrosa'></TD>";
                ...
                echo "      <TD Class=mostrar COLSPAN=2>7.8 Modalidad:<BR><SELECT name='modalidad' id='modalidad' >";
                echo "      </SELECT></TD>";
                ...
                echo "<TABLE CELLSPACING=10>";
                echo "<TH><INPUT Class=submit TYPE='SUBMIT' id='accion' name='accion' VALUE='Crear Reporte AG' ONCLICK='validar(this);'></TH>";         // Ordena almacenar los datos ingresados
                echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='boton' VALUE='Cancelar' ONCLICK='javascript:document.location.href = \"reportenegocio1.php\"'></TH>";  // Cancela la operación
                echo "<script>llenar_traficos();</script>";
                echo "<BR>";
                echo "</TABLE>";
        ...
        case 'Editar': {                                                    // Opcion para Adicionar Registros a la tabla
                ...
                echo "</IFRAME>";
                echo "</DIV>";
                echo "<DIV ID='find_texts' STYLE='display:hidden; position:absolute; border-width:3; border-color:#666666; border-style:solid;'>";
                echo "<IFRAME ID='find_texts_frame' SRC='blanco.html' MARGINWIDTH=0 MARGINHEIGHT=0 FRAMEBORDER='NO' SCROLLING='YES' STYLE='width:645; height:200'>";
                echo "</IFRAME>";
                echo "</DIV>";
                ...
                echo "<CENTER>";
                echo "<H3>$titulo</H3>";
                echo "<FORM METHOD=post NAME='adicionar' ACTION='reportenegocio1.php' ONSUBMIT='return validador();'>";// Plantilla de Edición
                ...
                echo "  <TD Class=mostrar style='text-align:left; vertical-align:top;'>Fecha del Reporte:<BR><CENTER><INPUT TYPE='TEXT' NAME='fchreporte' id='fchreporte' SIZE=12 VALUE='".$rs->Value('ca_fchreporte')."' READONLY></CENTER></TD>";
                ...
                echo "</TR>";
                echo "<TR>";
                echo "  <TD Class=captura style='vertical-align:top;'>&nbsp;&nbsp;&nbsp;&nbsp;
                    <SELECT ID='impoexpo' NAME='impoexpo' ONCHANGE='llenar_traficos();'>";
                for ($i=0; $i < count($imporexpor); $i++) {
                ...
                while (!$tm->Eof()) {
                    ...
                    echo "    <TD Class=mostrar style='text-align:right;' onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='terceros(\"find_contacto\",\"$pro\",\"findtercero1\");'><a><IMG src='graficos/lupa.gif' alt='Buscar' hspace='0' vspace='0'> Buscar</a></TD>";
                    echo "  </TR>";
                    echo "  <TR>";
                ...
                echo "    <TD Class=mostrar>8.1 Orden:<BR><INPUT TYPE='TEXT' NAME='orden_clie' id='orden_clie' VALUE='".$rs->Value('ca_orden_clie')."' SIZE=15 MAXLENGTH=255></TD>";
                echo "    <TD Class=mostrar style='text-align:right;' onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='terceros(\"find_contacto\",\"_cli\",\"findcontacto1\");'><a><IMG src='graficos/lupa.gif' alt='Buscar' hspace='0' vspace='0'> Buscar</a></TD>";
                echo "  </TR>";
                echo "  <TR>";
                ...
                echo "      <TD Class=mostrar>13. Modalidad:<BR><SELECT name='modalidad' id='modalidad' ONCHANGE='llenar_lineas();'>";
                echo "      </SELECT></TD>";
                echo "      <TD Class=mostrar COLSPAN=2>14. Línea Transporte:<BR><SELECT NAME='idlinea' id='idlinea'>";             // Llena el cuadro de lista con los valores de la tabla Transportistas
                echo "      </SELECT></TD>";
                echo "    </TR>";
                echo "    <TR HEIGHT=5>";
                echo "      <TD Class=invertir COLSPAN=4></TD>";
                ...
                echo "      <TD Class=mostrar>Tiempo de Crédito:<BR><CENTER><INPUT DISABLED TYPE='TEXT' NAME='tiempocredito' id='tiempocredito' VALUE='".($tm->Value('ca_diascredito')!=0?$tm->Value('ca_diascredito').' Días':'-')."' SIZE=18 MAXLENGTH=20></CENTER></TD>";
                echo "    </TR>";
                echo "    <TR HEIGHT=5>";
                echo "      <TD Class=invertir COLSPAN=4></TD>";
                echo "    </TR>";
                ...
                echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='boton' VALUE='Cancelar' ONCLICK='javascript:document.location.href = \"reportenegocio1.php?boton=Consultar\&id=$id\"'></TH>";  // Cancela la operación
                echo "<script>
                    window.onload =function() {
                    llenar_traficos();
                    elegir_traficos('".$rs->Value('ca_idtraorigen')."','".$rs->Value('ca_idtradestino')."');
                    elegir_puertos('".$rs->Value('ca_origen')."','".$rs->Value('ca_destino')."');
                    llenar_finales();
                    llenar_agentes();
                    elegir_item('idagente', '".$rs->Value('ca_idagente')."');

                  if ($('#idagente').val() != '".$rs->Value('ca_idagente')."') {
                   	 $('#todos_ag').attr('checked', true);
                   	 llenar_agentes();
                   	 elegir_item('idagente', '".$rs->Value('ca_idagente')."'); }
                
                elegir_item('modalidad', '".$rs->Value('ca_modalidad')."');
                llenar_lineas();
                elegir_item('idlinea', '".$rs->Value('ca_idlinea')."');
                elegir_item('continuacion', '".$rs->Value('ca_continuacion')."');
                elegir_item('idconsignar', '".$rs->Value('ca_idconsignar')."');
                elegir_item('tipo', '".$rs->Value('ca_tipobodega')."');
                llenar_bodegas();
                elegir_item('idbodega', '".$rs->Value('ca_idbodega')."');
                elegir_item('continuacion_dest', '".$rs->Value('ca_continuacion_dest')."');
                elegir_item('mastersame', '".$rs->Value('ca_mastersame')."');
                    };
                </script>";
                echo "<BR>";
                echo "</TABLE>";
        ...
        case 'Liquidar': {                                                    // Opcion para Consultar un solo registro
                ...
                echo "  ventana.style.display = \"block\";";
                ...
                require_once("menu.php");
                echo "<DIV ID='captura' STYLE='display:none; position:absolute; border-width:1; border-color:#445599; border-style:solid;'>";  // left:150; top:25; width:600; height:200
                ...
                echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='boton' VALUE=' Regresar ' ONCLICK='javascript:document.location.href = \"reportenegocio1.php?boton=Consultar\&id=$id\"'></TH>";  // Cancela la operación
                echo "</TABLE>";
                echo "</FORM>";
                echo "</CENTER>";
                //           echo "<P DIR='RTL'><A HREF=\"#\" ONCLICK='javascript:window.open(\"./help/$modulo.html\",\"Ayuda\",\"scrollbars=yes,width=600,height=400,top=200,left=150\")'><IMG SRC='./graficos/help.gif' border=0 ALT='Ayuda en Línea'><BR>Ayuda</A></P>";  // Link que proporciona la Ayuda en línea
    ...
    if (isset($status) and $status != 'false') {
		...
		*/
        ...
        echo "<script>document.location.href = '/traficos/listaStatus/modo/maritimo?reporte=$consecutivo';</script>";  // Retorna a la pantalla principal de la opción
            ...
            echo "<script>document.location.href = 'reportenegocio1.php?boton=Consultar\&id=$id';</script>";  // Retorna a la pantalla principal de la opción
        }else {
            echo "<script>document.location.href = 'reportenegocio1.php';</script>";  // Retorna a la pantalla principal de la opción
        }
}
...
function datos_basicos(&$visible,&$rs,&$tm) {
    ...
    echo "   <TR>";
    echo "  	<TD style='display: $visible;' Class=invertir style='text-align: right; vertical-align: bottom;' onclick='elegir(\"Editar\", \"".$rs->Value('ca_idreporte')."\");'>Editar Reporte:<br /><IMG src='./graficos/edit.gif' alt='Editar el Registro' border=0></TD>";
    echo "   </TR>";
    if (strlen($rs->Value('ca_usucerrado')) != 0) {
        echo "   <TR>";
        echo "  	<TD style='display: $visible;' Class=invertir style='text-align: right; vertical-align: bottom;'><B>".$rs->Value('ca_usucerrado')."</B><br />".$rs->Value('ca_fchcerrado')."</TD>";
        echo "   </TR>";
        echo "   <TR>";
        echo "  	<TD style='display: $visible;' Class=invertir style='text-align: right; vertical-align: bottom;'>Abrir el Caso:<br /><IMG src='./graficos/lock_close.gif' onclick='elegir(\"Abrir\", \"".$rs->Value('ca_idreporte')."\");' alt='Abrir nuevamente el Reporte' border=0></TD>";
        echo "   </TR>";
    }else {
        echo "   <TR>";
        echo "  	<TD style='display: $visible;' Class=invertir style='text-align: right; vertical-align: bottom;'>Cierre del Caso:<br /><IMG src='./graficos/lock_open.gif' onclick='elegir(\"Cerrar\", \"".$rs->Value('ca_idreporte')."\");' alt='Cerrar el Caso y Entregar Antecedentes' border=0></TD>";
        echo "   </TR>";
        echo "   <TR>";
        echo "  	<TD style='display: $visible;' Class=invertir style='text-align: right; vertical-align: bottom;'>Anular Reporte:<br /><IMG src='./graficos/no.gif' onclick='elegir(\"Anular\", \"".$rs->Value('ca_idreporte')."\");' alt='Anular y Ocultar el Reporte' border=0></TD>";
        echo "   </TR>";
    }
    echo "	 </TABLE></TD>";
...
function carga_arreglos(&$tm) {
    ...
    while (!$tm->Eof()) {
        $coma=($idtraficos!="")?",":"";
        $idtraficos.=$coma."'".$tm->Value('ca_idtrafico')."'";
        $nomtraficos.=$coma."'".$tm->Value('ca_nombre')."'";
        $tm->MoveNext();
    }
    echo "<script>var  idtraficos=new Array(".$idtraficos.");var nomtraficos=new Array(".$nomtraficos.");</script>";
    ...
    $tm->MoveFirst();
    while (!$tm->Eof()) {
        echo "<input type='hidden' name='idciudades' id='idciudades' title='".$tm->Value('ca_ciudad')."' value=".$tm->Value('ca_idciudad')." class='".$tm->Value('ca_idtrafico')."'>";
        $tm->MoveNext();
    }
    ...
    $tm->MoveFirst();
    while (!$tm->Eof()) {
        echo "<input type='hidden' name='idagentes' id='idagentes' title='".$tm->Value('ca_nombre')."' value=".$tm->Value('ca_idciudad')." class='".$tm->Value('ca_idtrafico')."'>";
    ...
    if (!$tm->Open("select ca_idlinea, ca_nombre, ca_transporte from vi_transporlineas order by ca_nombre")) { // Selecciona todos lo registros de la tabla Ciudades
        ...
        exit; }
    $tm->MoveFirst();
    echo "<INPUT TYPE='HIDDEN' NAME='aidlinea' title='PENDIENTE DE DEFINIR' VALUE=0 class='Aéreo'>";
    while ( !$tm->Eof()) {
        echo "<INPUT TYPE='HIDDEN' NAME='aidlinea' title='".$tm->Value('ca_nombre')."' VALUE=".$tm->Value('ca_idlinea')." class='".$tm->Value('ca_transporte')."'>";
        $tm->MoveNext();
    }
    ...
    while ( !$tm->Eof()) {
        if ($tm->Value('ca_tipo') == 'Coordinador Logístico' or $tm->Value('ca_tipo') == 'Operador Multimodal') {
            echo "<INPUT TYPE='HIDDEN' NAME='aidconsigna' title='".$tm->Value('ca_nombre')."' VALUE=".$tm->Value('ca_idbodega')." class='".$tm->Value('ca_tipo')."'>";
