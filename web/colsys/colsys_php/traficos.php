    ...
    switch(trim($accion)) {                                                    // Switch que evalua cual botòn de comando fue pulsado por el usuario
        case 'Guardar': {                                                      // El Botón Guardar fue pulsado
             if (!copy($bandera, './graficos/'.basename($bandera))) {
                ...
                if (!$rs->Open("insert into tb_traficos (ca_idtrafico, ca_nombre, ca_bandera, ca_idmoneda, ca_idgrupo) values(upper('$id'), '$nombre', '$bandera', '$idmoneda', $idgrupo)")) {
                    echo "<script>alert(\"".addslashes($rs->mErrMsg)."\");</script>";  // Muestra el mensaje de error
                    echo "<script>document.location.href = 'traficos.php';</script>";
                    exit;
                   }
        ...
        case 'Actualizar': {                                                   // El Botón Actualizar fue pulsado
             if (strlen($bandera)!=0) {
                 ...
                 $bandera = './graficos/'.basename($bandera);
                 if (!$rs->Open("update tb_traficos set ca_nombre = '$nombre', ca_bandera = '$bandera', ca_idmoneda = '$idmoneda', ca_idgrupo = $idgrupo where ca_idtrafico = '$id'")) {
                     echo "<script>alert(\"".addslashes($rs->mErrMsg)."\");</script>";  // Muestra el mensaje de error
                     echo "<script>document.location.href = 'traficos.php';</script>";
                     exit;
                    }
                 ...
                 if (!$rs->Open("update tb_traficos set ca_nombre = '$nombre', ca_idmoneda = '$idmoneda', ca_idgrupo = $idgrupo where ca_idtrafico = '$id'")) {
                     echo "<script>alert(\"".addslashes($rs->mErrMsg)."\");</script>";  // Muestra el mensaje de error
                     echo "<script>document.location.href = 'traficos.php';</script>";
                     exit;
                    }
