...
// Updated:       2005-06-01                                                  \\
//                                                                            \\
// Descripci�n:   M�dulo para la creaci�n de Reportes de Negocio.             \\
//                                                                            \\
//                                                                            \\
...
$columnas = array("N�mero de Reporte"=>"ca_consecutivo", "Nombre del Cliente"=>"ca_nombre_cli", "Nombre del Consignatario"=>"ca_nombre_con", "Mis Reportes"=>"ca_login", "Nombre del Proveedor"=>"ca_nombre_pro", "No.Orden Proveedor"=>"ca_orden_prov", "No.Orden Cliente"=>"ca_orden_clie", "No. Cotizaci�n"=>"ca_idcotizacion", "Descripci�n Mercancia"=>"ca_mercancia_desc", "Vendedor"=>"ca_login", "Borradores"=>"ca_contenido", "Tr�ficos"=>"ca_traorigen", "Puerto"=>"ca_ciuorigen");                        // Arreglo con las opciones de busqueda
$siono = array("S�","No");
...
require_once("menu.php");
    ...
    $vista_1 = ($nivel >= 0)?'visible':'hidden'; 					// Habilita la opci�n para creaci�n de reportes AG
    echo "<TH>";
    echo "   <TABLE CELLSPACING=1>";
    echo "      <TR>";
    echo "          <TD><IMG src='./graficos/new_ag.gif' alt='Crear Reporte AG' border=0 onclick='elegir(\"Reporte_Ag\", 0, 0);' style='visibility: $vista_1;'></TH></TD>";  // Bot�n para la creaci�n de un Registro Nuevo
    echo "          <TD><IMG src='./graficos/new.gif' alt='Crear Reporte de Negocio' border=0 onclick='elegir(\"Adicionar\", 0, 0);'></TH></TD>";  // Bot�n para la creaci�n de un Registro Nuevo
    echo " 	 </TR>";
    echo "   </TABLE>";
    echo "</TH>";
    echo "<TR>";
    ...
    echo "<TR>";
    echo "  <TD Class=listar WIDTH=160>Con Continuaci�n/Viaje:&nbsp;<INPUT TYPE=CHECKBOX NAME='continuacion' ONCLICK='habilitar(this);'></TD>";
    $tm =& DlRecordset::NewRecordset($conn);
    if (!$tm->Open("select ca_idciudad, ca_ciudad from vi_ciudades where ca_idtrafico = 'CO-057' and ca_puerto not in ('Mar�timo','Ambos') order by ca_ciudad")) {       // Selecciona todos lo registros de la tabla ciudades
    ...
    while (!$rs->Eof() and !$rs->IsEmpty()) {                                                      // Lee la totalidad de los registros obtenidos en la instrucci�n Select
        if ($ano_mem != $rs->Value('ca_fchreporte')){
                $ano_mem = intval(substr($rs->Value('ca_fchreporte'),0,4));
                echo "<TR>";
                echo "  <TH Class=titulo COLSPAN=7>A�O $ano_mem</TH>";
                echo "</TR>";
        }
        echo "<TR>";
...
require_once("menu.php");
    ...
    if ($opcion != 'ca_contenido'){
       ...
       if (!$rs->Open("select * from vi_reportes $condicion")) {                       // Selecciona todos lo registros de la tabla Ino-Mar�timo
           echo "<script>alert(\"".addslashes($rs->mErrMsg)."\");</script>";      // Muestra el mensaje de error
           echo "<script>document.location.href = 'entrada.php';</script>";
           exit; }
       ...
       while (!$rs->Eof() and !$rs->IsEmpty()) {                                                      // Lee la totalidad de los registros obtenidos en la instrucci�n Select
          ...
          echo "    <TD Class=invertir style='font-weight:bold;'>Cotizaci�n</TD>";
          echo "  </TR>";
          echo "  <TR>";
       ...
       if (!$rs->Open("select oid, * from tb_repborrador where ca_usucreado = '$usuario' and ca_contenido like '%$criterio%' order by ca_fchcreado DESC")) {         // Selecciona todos lo registros de la tabla Borradores de Reporte
           ...
           exit; }
       echo "<TH>Usuario</TH>";
       echo "<TH COLSPAN=2>Fecha y Hora de Creaci�n</TH>";
       $vista_1 = ($nivel >= 0)?'visible':'hidden'; 					// Habilita la opci�n para creaci�n de reportes AG
       echo "<TH>";
       echo "   <TABLE CELLSPACING=1>";
       echo "   <TR>";
    ...
    echo "</TABLE><BR>";
    echo "<TABLE CELLSPACING=10>";
    echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='boton' VALUE='Regresar' ONCLICK='javascript:document.location.href = \"reportenegocio.php\"'></TH>";  // Cancela la operaci�n
    echo "</TABLE>";
    echo "</FORM>";
    echo "</CENTER>";
//    echo "<P DIR='RTL'><A HREF=\"#\" ONCLICK='javascript:window.open(\"./help/$modulo.html\",\"Ayuda\",\"scrollbars=yes,width=600,height=400,top=200,left=150\")'><IMG SRC='./graficos/help.gif' border=0 ALT='Ayuda en L�nea'><BR>Ayuda</A></P>";  // Link que proporciona la Ayuda en l�nea
...
elseif (isset($boton)) {                                                       // Switch que evalua cual bot�n de comando fue pulsado por el usuario
    switch(trim($boton)) {
        case 'Consultar': {                                                    // Opcion para Consultar un solo registro
             ...
             if (!$rs->Open("select * from vi_reportes where ca_idreporte = $id $condicion")) {                       // Selecciona todos lo registros de la tabla Ino-Mar�timo
                 echo "<script>alert(\"".addslashes($rs->mErrMsg)."\");</script>";      // Muestra el mensaje de error
                 echo "<script>document.location.href = 'entrada.php';</script>";
                 exit; }
             ...
             echo "                 det_mem = prompt('Por favor ingrese un breve detalle del motivo de la anulaci�n:');";
...
require_once("menu.php");
             ...
             if ($rs->Value('ca_seguro')== "S�") {
                 ...
                 echo "  <TR>";
                 echo "    <TD Class=listar COLSPAN=3 style='text-align:center; font-weight:bold;'>INFORMACI�N PARA LA ASEGURADORA</TD>";
                 echo "  </TR>";
                 echo "  <TR>";
                 echo "    <TD Class=listar><B>20.1 Valor Asegurado:</B><BR>".formatNumber($sg->Value('ca_vlrasegurado'),3)." ".$sg->Value('ca_idmoneda_vlr')."</TD>";
                 echo "    <TD Class=listar><B>20.2 Obtenci�n P�liza:</B><BR>".formatNumber($sg->Value('ca_obtencionpoliza'),3)." ".$sg->Value('ca_idmoneda_pol')."</TD>";
             ...
             echo "<TR>";
             echo "  <TD Class=titulo COLSPAN=5><CENTER><TABLE WIDTH=100% CELLSPACING=1 BORDER=0>";
             echo "  <TD Class=invertir COLSPAN=8 style='text-align:center; font-weight:bold;'>RELACI�N DE RECARGOS</TD>";
             echo "  <TR>";
             echo "    <TD Class=titulo><B>$sub_mem</B></TD>";
             echo "    <TD Class=titulo><B>Aplicaci�n</B></TD>";
             echo "    <TD Class=titulo COLSPAN=2><B>Neta / Min</B></TD>";
             echo "    <TD Class=titulo COLSPAN=2><B>Reportar / Min</B></TD>";
             echo "    <TD Class=titulo COLSPAN=2><B>Cobrar / Min</B></TD>";
             echo "  </TR>";
             ...
             echo "  <TD Class=listar><B>Elabor� :</B><BR><CENTER>$usu_mem</CENTER></TD>";
             ...
             $tm =& DlRecordset::NewRecordset($conn);
             if (!$tm->Open("select * from vi_emails where ca_tipo = 'Rep.Mar�timoExterior' and ca_idcaso in (select ca_idreporte from tb_reportes where ca_consecutivo in (select ca_consecutivo from tb_reportes where ca_idreporte = '$id')) order by ca_fchenvio DESC")) { // Selecciona todos lo registros de la tabla Emails
                echo "<script>alert(\"".addslashes($tm->mErrMsg)."\");</script>";      // Muestra el mensaje de error
                echo "<script>document.location.href = 'reportenegocio.php';</script>";
                exit; }
             ...
             echo "<TH>Enviado</TH>";
             echo "<TH WIDTH=35></TH>";  // Bot�n para la creaci�n de un Registro Nuevo
             while (!$tm->Eof()) {
                echo "<TR>";
                ...
                echo "  <TD Class=mostrar><CENTER><IMG src='./graficos/mail.gif' alt='Ver correo electr�nico' border=0 onclick='javascript:window.open(\"ventanas.php?opcion=Email\&id=".$tm->Value('ca_idemail')."\",\"Email\",\"scrollbars=yes,width=650,height=400,top=200,left=150\")'></CENTER></TD>";
                echo "</TR>";
                $tm->MoveNext();
             }
             echo "<TR HEIGHT=5>";
             ...
             echo "<TABLE WIDTH=620 CELLSPACING=1>";                                   // un boton de comando definido para hacer mantemientos
             echo "<TR>";
             echo "  <TD Class=invertir style='text-align:center; font-weight:bold;' COLSPAN=6>Relaci�n de Status Enviados</TD>";
             echo "</TR>";
...
//           include_once 'include/seguridad.php';                             // Control de Acceso al m�dulo
             ...
             if (!$tm->Open("select ca_email, ca_login from vi_usuarios where ca_cargo like '%Operaciones Maritimas%'")) {          // Selecciona los correos de la tabla Parametros
                 echo "<script>alert(\"".addslashes($tm->mErrMsg)."\");</script>";      // Muestra el mensaje de error
                 echo "<script>document.location.href = 'reportenegocio.php';</script>";
                 exit; }
             echo "<HEAD>";
             ...
             echo "<TR>";
             echo "  <TD Class=titulo style='text-align:left; vertical-align:top;'>Informaci�n :</TD>";
             ...
             echo "</TR>";
             echo "<TR>";
             echo "  <TD Class=mostrar COLSPAN=4>6. Descripci�n de la Mercanc�a:<BR><TEXTAREA NAME='mercancia_desc' WRAP=virtual ROWS=3 COLS=93></TEXTAREA><BR>�Es Mercanc�a Peligrosa? <INPUT TYPE=CHECKBOX NAME='mcia_peligrosa'><IMG SRC='./graficos/nuevo.gif' border=0 ALT='Marque esta opci�n para indicar que S� es Mercancia Peligrosa'></TD>";
             echo "</TR>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>7.3 Direcci�n:<BR><INPUT ID=direccion_pro_1 READONLY TYPE='TEXT' NAME='proveedor[]' SIZE=40 MAXLENGTH=80></TD>";
             echo "  </TR>";
             echo "  <TR>";
             echo "    <TD Class=mostrar>7.4 Tel�fono:<BR><INPUT ID=telefonos_pro_1 READONLY TYPE='TEXT' NAME='proveedor[]' SIZE=23 MAXLENGTH=30></TD>";
             echo "    <TD Class=mostrar>7.5 Fax:<BR><INPUT ID=fax_pro_1 READONLY TYPE='TEXT' NAME='proveedor[]' SIZE=23 MAXLENGTH=30></TD>";
             echo "    <TD Class=mostrar COLSPAN=2>7.6 Correo Electr�nico:<BR><INPUT ID=email_pro_1 READONLY TYPE='TEXT' NAME='proveedor[]' SIZE=30 MAXLENGTH=40></TD>";
             echo "  </TR>";
             echo "  <TR>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>8.3 Direcci�n:<BR><INPUT ID=direccion_cli READONLY TYPE='TEXT' NAME='cliente[]' SIZE=40 MAXLENGTH=80></TD>";
             echo "  </TR>";
             echo "  <TR>";
             echo "    <TD Class=mostrar>8.4 Tel�fono:<BR><INPUT ID=telefonos_cli READONLY TYPE='TEXT' NAME='cliente[]' SIZE=23 MAXLENGTH=30></TD>";
             echo "    <TD Class=mostrar>8.5 Fax:<BR><INPUT ID=fax_cli READONLY TYPE='TEXT' NAME='cliente[]' SIZE=23 MAXLENGTH=30></TD>";
             echo "    <TD Class=mostrar COLSPAN=2>8.6 Correo Electr�nico:<BR><INPUT ID=email_cli READONLY TYPE='TEXT' NAME='cliente[]' SIZE=30 MAXLENGTH=40></TD>";
             echo "  </TR>";
             echo "  <TR>";
             ...
             echo "  <TD Class=titulo ROWSPAN=2 style='text-align:left; vertical-align:top;'><IMG id='cons_btn' SRC='./graficos/cerrado.gif' border=0 ALT='Pulse aqu� para extender el cuadro' onclick='extender(this);'>&nbsp;Consignatario:<BR></TD>";
             ...
             echo "    <TD Class=mostrar>9.1.1 Enviar Informaci�n:<BR>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_cons' TYPE='radio' VALUE = 'S�'>S�&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_cons' TYPE='radio' VALUE = 'No' CHECKED>No</TD>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>9.1.3 Direcci�n:<BR><INPUT ID=direccion_con READONLY TYPE='TEXT' NAME='consignatario[]' SIZE=40 MAXLENGTH=80></TD>";
             echo "  </TR>";
             echo "  <TR>";
             echo "    <TD Class=mostrar>9.1.4 Tel�fono:<BR><INPUT ID=telefonos_con READONLY TYPE='TEXT' NAME='consignatario[]' SIZE=23 MAXLENGTH=30></TD>";
             echo "    <TD Class=mostrar>9.1.5 Fax:<BR><INPUT ID=fax_con READONLY TYPE='TEXT' NAME='consignatario[]' SIZE=23 MAXLENGTH=30></TD>";
             echo "    <TD Class=mostrar COLSPAN=2>9.1.6 Correo Electr�nico:<BR><INPUT ID=email_con READONLY TYPE='TEXT' NAME='consignatario[]' SIZE=30 MAXLENGTH=40></TD>";
             echo "  </TR>";
             echo "  <TR>";
             ...
             echo "  <TD Class=titulo ROWSPAN=2 style='text-align:left; vertical-align:top;'><IMG id='noti_btn' SRC='./graficos/cerrado.gif' border=0 ALT='Pulse aqu� para extender el cuadro' onclick='extender(this);'>&nbsp;Notify:</TD>";
             ...
             echo "    <TD Class=mostrar>9.2.1 Enviar Informaci�n:<BR>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_noti' TYPE='radio' VALUE = 'S�'>S�&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_noti' TYPE='radio' VALUE = 'No' CHECKED>No</TD>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>9.2.3 Direcci�n:<BR><INPUT ID=direccion_not READONLY TYPE='TEXT' NAME='notify[]' SIZE=40 MAXLENGTH=80></TD>";
             echo "  </TR>";
             echo "  <TR>";
             echo "    <TD Class=mostrar>9.2.4 Tel�fono:<BR><INPUT ID=telefonos_not READONLY TYPE='TEXT' NAME='notify[]' SIZE=23 MAXLENGTH=30></TD>";
             echo "    <TD Class=mostrar>9.2.5 Fax:<BR><INPUT ID=fax_not READONLY TYPE='TEXT' NAME='notify[]' SIZE=23 MAXLENGTH=30></TD>";
             echo "    <TD Class=mostrar COLSPAN=2>9.2.6 Correo Electr�nico:<BR><INPUT ID=email_not READONLY TYPE='TEXT' NAME='notify[]' SIZE=30 MAXLENGTH=40></TD>";
             echo "  </TR>";
             echo "  <TR>";
             ...
             echo "  <TD Class=titulo ROWSPAN=2 style='text-align:left; vertical-align:top;'><IMG id='mast_btn' SRC='./graficos/cerrado.gif' border=0 ALT='Pulse aqu� para extender el cuadro' onclick='extender(this);'>&nbsp;Consigna.Master:</TD>";
             ...
             echo "    <TD Class=mostrar>9.3.1 Enviar Informaci�n:<BR>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_mast' TYPE='radio' VALUE = 'S�'>S�&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_mast' TYPE='radio' VALUE = 'No' CHECKED>No</TD>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>9.3.3 Direcci�n:<BR><INPUT ID=direccion_mas READONLY TYPE='TEXT' NAME='master[]' SIZE=40 MAXLENGTH=80></TD>";
             echo "  </TR>";
             echo "  <TR>";
             echo "    <TD Class=mostrar>9.3.4 Tel�fono:<BR><INPUT ID=telefonos_mas READONLY TYPE='TEXT' NAME='master[]' SIZE=23 MAXLENGTH=30></TD>";
             echo "    <TD Class=mostrar>9.3.5 Fax:<BR><INPUT ID=fax_mas READONLY TYPE='TEXT' NAME='master[]' SIZE=23 MAXLENGTH=30></TD>";
             echo "    <TD Class=mostrar COLSPAN=2>9.3.6 Correo Electr�nico:<BR><INPUT ID=email_mas READONLY TYPE='TEXT' NAME='master[]' SIZE=30 MAXLENGTH=40></TD>";
             echo "  </TR>";
             echo "  </TABLE></TD>";
             echo "</TR>";
             echo "<TR HEIGHT=5>";
             ...
             echo "  <TD Class=titulo ROWSPAN=2 style='text-align:left; vertical-align:top;'><IMG id='repr_btn' SRC='./graficos/cerrado.gif' border=0 ALT='Pulse aqu� para extender el cuadro' onclick='extender(this);'>&nbsp;Representante:<BR></TD>";
             ...
             echo "    <TD Class=mostrar>10.1 Enviar Informaci�n:<BR>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_repr' TYPE='radio' VALUE = 'S�'>S�&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_repr' TYPE='radio' VALUE = 'No' CHECKED>No</TD>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>10.3 Direcci�n:<BR><INPUT ID=direccion_rep READONLY TYPE='TEXT' NAME='representante[]' SIZE=40 MAXLENGTH=80></TD>";
             echo "  </TR>";
             echo "  <TR>";
             echo "    <TD Class=mostrar>10.4 Tel�fono:<BR><INPUT ID=telefonos_rep READONLY TYPE='TEXT' NAME='representante[]' SIZE=23 MAXLENGTH=30></TD>";
             echo "    <TD Class=mostrar>10.5 Fax:<BR><INPUT ID=fax_rep READONLY TYPE='TEXT' NAME='representante[]' SIZE=23 MAXLENGTH=30></TD>";
             echo "    <TD Class=mostrar COLSPAN=2>10.6 Correo Electr�nico:<BR><INPUT ID=email_rep READONLY TYPE='TEXT' NAME='representante[]' SIZE=30 MAXLENGTH=40></TD>";
             echo "  </TR>";
             echo "  </TABLE></TD>";
             echo "</TR>";
             echo "<TR HEIGHT=5>";
             ...
             echo "      </SELECT></TD>";
             echo "      <TD Class=mostrar COLSPAN=3>14. L�nea Transporte:<BR><SELECT NAME='idlinea'>";             // Llena el cuadro de lista con los valores de la tabla Transportistas
             echo "      </SELECT></TD>";
             echo "    </TR>";
             echo "    <TR HEIGHT=5>";
             echo "      <TD Class=invertir COLSPAN=4></TD>";
             ...
             echo "    <TR>";
             echo "      <TD Class=mostrar>15. Colmas S.I.A. Ltda:<BR><CENTER><INPUT NAME='colmas' TYPE='radio' VALUE = 'S�' CHECKED>S�&nbsp;&nbsp;&nbsp;&nbsp;<INPUT NAME='colmas' TYPE='radio' VALUE = 'No'>No</CENTER></TD>";
             echo "      <TD Class=mostrar>16. Seguro:<BR><CENTER><INPUT NAME='seguro' TYPE='radio' VALUE = 'S�' CHECKED>S�&nbsp;&nbsp;&nbsp;&nbsp;<INPUT NAME='seguro' TYPE='radio' VALUE = 'No'>No</CENTER></TD>";
             echo "      <TD Class=mostrar>17. Lib. Autom�tica:<BR><CENTER><INPUT ID=si NAME='liberacion' DISABLED TYPE='radio' VALUE = 'S�'>S�&nbsp;&nbsp;&nbsp;<INPUT ID=no NAME='liberacion' DISABLED TYPE='radio' VALUE = 'No'>No</CENTER></TD>";
             echo "      <TD Class=mostrar>Tiempo de Cr�dito:<BR><CENTER><INPUT DISABLED TYPE='TEXT' NAME='tiempocredito' VALUE='-' SIZE=18 MAXLENGTH=20></CENTER></TD>";
             echo "    </TR>";
             echo "    <TR HEIGHT=5>";
             echo "      <TD Class=invertir COLSPAN=4></TD>";
             echo "    </TR>";
             echo "    <TR>";
             echo "      <TD Class=listar WIDTH=175>18.1 Continuaci�n/Viaje:</TD>";
             ...
             echo "    <TR>";
             echo "      <TD Class=mostrar>19.1 Consignar HAWB/HBL a :</TD>";                                       // Llena el cuadro de lista con las zonas o dep�sitos aduaneros
             echo "      <TD Class=mostrar COLSPAN=3><SELECT NAME='idconsignar'></SELECT></TD>";
             echo "    </TR>";
             echo "    <TR>";
             echo "      <TD Class=mostrar>19.2 Trasladar a :</TD>";                                           // Llena el cuadro de lista con las zonas o dep�sitos aduaneros
             echo "      <TD Class=mostrar COLSPAN=3><SELECT NAME='tipo' ONCHANGE='llenar_bodegas();'></SELECT></TD>";
             echo "    </TR>";
             echo "    <TR>";
             echo "      <TD Class=mostrar>19.3&nbsp;Igualar&nbsp;Master/Hijo:&nbsp;<SELECT NAME='mastersame'><OPTION VALUE='No'>No</OPTION><OPTION VALUE='S�'>S�</OPTION></SELECT></TD>";
             echo "      <TD Class=mostrar COLSPAN=4><SELECT NAME='idbodega'></SELECT></TD>";                   // Llena el cuadro de lista con las zonas o dep�sitos aduaneros
             echo "    </TR>";
             echo "  </TABLE></TD>";
             ...
             echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='boton' VALUE='Cancelar' ONCLICK='javascript:document.location.href = \"reportenegocio.php\"'></TH>";  // Cancela la operaci�n
...
//           include_once 'include/seguridad.php';                             // Control de Acceso al m�dulo
             ...
             echo "  <TD Class=invertir COLSPAN=2 ROWSPAN=3 STYLE='vertical-align:top'>Copiar mensaje a: <IMG SRC='./graficos/nuevo.gif' border=0 ALT='Seleccionar los Contactos involucrados con este embarque'><BR><B>El reporte AG s�lo ser� enviado a los buzones de dominio @coltrans.com.co</B><TABLE WIDTH=100% CELLSPACING=1 CELLPADDING=0 BORDER=0>";
             $z=0;
             for ($i=1; $i<=11; $i++){
                echo "  <TR>";
             ...
             echo "  </TABLE></TD>";
             echo "</TR>";
             $saludo = (date('H')<12)?"buenos d�as":((date('H')>18)?"buenas noches":"buenas tardes");
			 echo "<TR>";
...
//           include_once 'include/seguridad.php';                             // Control de Acceso al m�dulo
             ...
             echo "<CENTER>";
             echo "<H3>$titulo</H3>";
             echo "<FORM METHOD=post NAME='adicionar' ACTION='reportenegocio.php' ONSUBMIT='return validador();'>";// Plantilla de Edici�n
             ...
             echo "  <TD Class=mostrar style='text-align:left; vertical-align:top;'>Cotizaci�n No.:<BR><CENTER><INPUT TYPE='TEXT' NAME='idcotizacion' VALUE='".$rs->Value('ca_idcotizacion')."' ONBLUR='valores(this);' SIZE=11 MAXLENGTH=10></CENTER></TD>";
             echo "  <TD Class=invertir style='text-align:left; vertical-align:top;'><TABLE WIDTH=100% CELLSPACING=0>";
             echo "  	<TR><TD Class=mostrar style='text-align:left; vertical-align:bottom;' onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\"  onclick='terceros(\"find_texts\",\"impreporte\",\"findreporte\");'><CENTER><IMG alt='Click aqu� para Buscar Reporte' src='graficos/importar.gif' hspace='0' vspace='0'>&nbsp;Importar&nbsp;un&nbsp;Reporte</CENTER></TD></TR>";
             echo "  	<TR><TD Class=mostrar style='text-align:left; vertical-align:bottom;' onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\"  onclick='terceros(\"find_contacto\",\"_pro_1\",\"findcotizacion\");'><CENTER><IMG alt='Click aqu� para Buscar Cotizaci�n' src='graficos/lupa.gif' hspace='0' vspace='0'>&nbsp;Buscar&nbsp;Cotizaci�n</CENTER></TD></TR>";
             echo "  </TABLE></TD>";
             echo "</TR>";
             ...
             echo "</TR>";
             echo "<TR>";
             echo "  <TD Class=mostrar COLSPAN=4>6. Descripci�n de la Mercanc�a:<BR><TEXTAREA NAME='mercancia_desc' WRAP=virtual ROWS=3 COLS=93>".$rs->Value('ca_mercancia_desc')."</TEXTAREA><BR>�Es Mercanc�a Peligrosa? <INPUT TYPE=CHECKBOX NAME='mcia_peligrosa' ".(($rs->Value("ca_mcia_peligrosa")=='t')?"CHECKED":"")."><IMG SRC='./graficos/nuevo.gif' border=0 ALT='Marque esta opci�n para indicar que S� es Mercancia Peligrosa'></TD>";
             echo "</TR>";
             $cadena = (trim(strlen($rs->Value('ca_idproveedor'))) != 0)?"ca_idtercero in (".str_replace("|",",",$rs->Value('ca_idproveedor')).")":"false";
             ...
             while (!$tm->Eof()) {
                ...
                echo "    <TD Class=mostrar COLSPAN=2>7.3 Direcci�n:<BR><INPUT ID=direccion$pro READONLY TYPE='TEXT' NAME='proveedor[]' VALUE='".$tm->Value('ca_direccion')."' SIZE=40 MAXLENGTH=80></TD>";
                echo "  </TR>";
                echo "  <TR>";
                echo "    <TD Class=mostrar>7.4 Tel�fono:<BR><INPUT ID=telefonos$pro READONLY TYPE='TEXT' NAME='proveedor[]' VALUE='".$tm->Value('ca_telefonos')."' SIZE=23 MAXLENGTH=30></TD>";
                ...
                echo "    <TD Class=mostrar COLSPAN=2>7.6 Correo Electr�nico:<BR><INPUT ID=email$pro READONLY TYPE='TEXT' NAME='proveedor[]' VALUE='".$tm->Value('ca_email')."' SIZE=30 MAXLENGTH=40></TD>";
                echo "  </TR>";
                echo "  <TR>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>8.3 Direcci�n:<BR><INPUT ID=direccion_cli READONLY TYPE='TEXT' NAME='cliente[]' VALUE='".str_replace ("|"," ",$rs->Value('ca_direccion_cli'))."' SIZE=40 MAXLENGTH=80></TD>";
             echo "  </TR>";
             echo "  <TR>";
             echo "    <TD Class=mostrar>8.4 Tel�fono:<BR><INPUT ID=telefonos_cli READONLY TYPE='TEXT' NAME='cliente[]' VALUE='".$rs->Value('ca_telefonos_cli')."' SIZE=23 MAXLENGTH=30></TD>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>8.6 Correo Electr�nico:<BR><INPUT ID=email_cli READONLY TYPE='TEXT' NAME='cliente[]' VALUE='".$rs->Value('ca_email_cli')."' SIZE=30 MAXLENGTH=40></TD>";
             echo "  </TR>";
             echo "  <TR>";
             ...
             echo "    <TD Class=mostrar>9.1.1 Enviar Informaci�n:<BR>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_cons' TYPE='radio' VALUE = 'S�' ".($rs->Value('ca_informar_cons')=='S�'?'CHECKED':'').">S�&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_cons' TYPE='radio' VALUE = 'No' ".($rs->Value('ca_informar_cons')=='No'?'CHECKED':'').">No</TD>";
             echo "    <TD Class=mostrar style='text-align:right;' onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='terceros(\"find_contacto\",\"_con\",\"findtercero\");'><a><IMG src='graficos/lupa.gif' alt='Buscar' hspace='0' vspace='0'> Buscar</a></TD>";
             echo "  </TR>";
             echo "  <TR>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>9.1.3 Direcci�n:<BR><INPUT ID=direccion_con READONLY TYPE='TEXT' NAME='consignatario[]' VALUE='".$rs->Value('ca_direccion_con')."' SIZE=40 MAXLENGTH=80></TD>";
             echo "  </TR>";
             echo "  <TR>";
             echo "    <TD Class=mostrar>9.1.4 Tel�fono:<BR><INPUT ID=telefonos_con READONLY TYPE='TEXT' NAME='consignatario[]' VALUE='".$rs->Value('ca_telefonos_con')."' SIZE=23 MAXLENGTH=30></TD>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>9.1.6 Correo Electr�nico:<BR><INPUT ID=email_con READONLY TYPE='TEXT' NAME='consignatario[]' VALUE='".$rs->Value('ca_email_con')."' SIZE=30 MAXLENGTH=40></TD>";
             echo "  </TR>";
             echo "  <TR>";
             ...
             echo "  <TD Class=titulo ROWSPAN=2 style='text-align:left; vertical-align:top;'><IMG id='noti_btn' SRC='./graficos/cerrado.gif' border=0 ALT='Pulse aqu� para extender el cuadro' onclick='extender(this);'>&nbsp;Notify:<BR></TD>";
             ...
             echo "    <TD Class=mostrar>9.2.1 Enviar Informaci�n:<BR>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_noti' TYPE='radio' VALUE = 'S�' ".($rs->Value('ca_informar_noti')=='S�'?'CHECKED':'').">S�&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_noti' TYPE='radio' VALUE = 'No' ".($rs->Value('ca_informar_noti')=='No'?'CHECKED':'').">No</TD>";
             echo "    <TD Class=mostrar style='text-align:right;' onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='terceros(\"find_contacto\",\"_not\",\"findtercero\");'><a><IMG src='graficos/lupa.gif' alt='Buscar' hspace='0' vspace='0'> Buscar</a></TD>";
             echo "  </TR>";
             echo "  <TR>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>9.2.3 Direcci�n:<BR><INPUT ID=direccion_not READONLY TYPE='TEXT' NAME='notify[]' VALUE='".$rs->Value('ca_direccion_not')."' SIZE=40 MAXLENGTH=80></TD>";
             echo "  </TR>";
             echo "  <TR>";
             echo "    <TD Class=mostrar>9.2.4 Tel�fono:<BR><INPUT ID=telefonos_not READONLY TYPE='TEXT' NAME='notify[]' VALUE='".$rs->Value('ca_telefonos_not')."' SIZE=23 MAXLENGTH=30></TD>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>9.2.6 Correo Electr�nico:<BR><INPUT ID=email_not READONLY TYPE='TEXT' NAME='notify[]' VALUE='".$rs->Value('ca_email_not')."' SIZE=30 MAXLENGTH=40></TD>";
             echo "  </TR>";
             echo "  <TR>";
             ...
             echo "  <TD Class=titulo ROWSPAN=2 style='text-align:left; vertical-align:top;'><IMG id='mast_btn' SRC='./graficos/cerrado.gif' border=0 ALT='Pulse aqu� para extender el cuadro' onclick='extender(this);'>&nbsp;Consigna.Master:<BR></TD>";
             ...
             echo "    <TD Class=mostrar>9.3.1 Enviar Informaci�n:<BR>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_mast' TYPE='radio' VALUE = 'S�' ".($rs->Value('ca_informar_mast')=='S�'?'CHECKED':'').">S�&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_mast' TYPE='radio' VALUE = 'No' ".($rs->Value('ca_informar_mast')=='No'?'CHECKED':'').">No</TD>";
             echo "    <TD Class=mostrar style='text-align:right;' onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='terceros(\"find_contacto\",\"_mas\",\"findtercero\");'><a><IMG src='graficos/lupa.gif' alt='Buscar' hspace='0' vspace='0'> Buscar</a></TD>";
             echo "  </TR>";
             echo "  <TR>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>9.3.3 Direcci�n:<BR><INPUT ID=direccion_mas READONLY TYPE='TEXT' NAME='master[]' VALUE='".$rs->Value('ca_direccion_mas')."' SIZE=40 MAXLENGTH=80></TD>";
             echo "  </TR>";
             echo "  <TR>";
             echo "    <TD Class=mostrar>9.3.4 Tel�fono:<BR><INPUT ID=telefonos_mas READONLY TYPE='TEXT' NAME='master[]' VALUE='".$rs->Value('ca_telefonos_mas')."' SIZE=23 MAXLENGTH=30></TD>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>9.3.6 Correo Electr�nico:<BR><INPUT ID=email_mas READONLY TYPE='TEXT' NAME='master[]' VALUE='".$rs->Value('ca_email_mas')."' SIZE=30 MAXLENGTH=40></TD>";
             echo "  </TR>";
             echo "  </TABLE></TD>";
             echo "</TR>";
             echo "<TR HEIGHT=5>";
             ...
             echo "    <TD Class=mostrar>10.1 Enviar Informaci�n:<BR>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_repr' TYPE='radio' VALUE = 'S�' ".($rs->Value('ca_informar_repr')=='S�'?'CHECKED':'').">S�&nbsp;&nbsp;&nbsp;<INPUT NAME='informar_repr' TYPE='radio' VALUE = 'No' ".($rs->Value('ca_informar_repr')=='No'?'CHECKED':'').">No</TD>";
             echo "    <TD Class=mostrar style='text-align:right;' onMouseOver=\"uno(this,'CCCCCC');\" onMouseOut=\"dos(this,'F0F0F0');\" onclick='terceros(\"find_contacto\",\"_rep\",\"findtercero\");'><a><IMG src='graficos/lupa.gif' alt='Buscar' hspace='0' vspace='0'> Buscar</a></TD>";
             echo "  </TR>";
             echo "  <TR>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>10.3 Direcci�n:<BR><INPUT ID=direccion_rep READONLY TYPE='TEXT' NAME='representante[]' VALUE='".$rs->Value('ca_direccion_rep')."' SIZE=40 MAXLENGTH=80></TD>";
             echo "  </TR>";
             echo "  <TR>";
             echo "    <TD Class=mostrar>10.4 Tel�fono:<BR><INPUT ID=telefonos_rep READONLY TYPE='TEXT' NAME='representante[]' VALUE='".$rs->Value('ca_telefonos_rep')."' SIZE=23 MAXLENGTH=30></TD>";
             ...
             echo "    <TD Class=mostrar COLSPAN=2>10.6 Correo Electr�nico:<BR><INPUT ID=email_rep READONLY TYPE='TEXT' NAME='representante[]' VALUE='".$rs->Value('ca_email_rep')."' SIZE=30 MAXLENGTH=40></TD>";
             echo "  </TR>";
             echo "  </TABLE></TD>";
             echo "</TR>";
             echo "<TR HEIGHT=5>";
             ...
             echo "      <TD Class=mostrar>13. Modalidad:<BR><SELECT NAME='modalidad' ONCHANGE='llenar_lineas();'>";
             echo "      </SELECT></TD>";
             echo "      <TD Class=mostrar COLSPAN=2>14. L�nea Transporte:<BR><SELECT NAME='idlinea'>";             // Llena el cuadro de lista con los valores de la tabla Transportistas
             echo "      </SELECT></TD>";
             echo "    </TR>";
             echo "    <TR HEIGHT=5>";
             echo "      <TD Class=invertir COLSPAN=4></TD>";
             ...
             echo "    <TR>";
             echo "      <TD Class=mostrar>15. Colmas S.I.A. Ltda:<BR><CENTER><INPUT ID='chk_colmas_0' NAME='colmas' TYPE='radio' VALUE = 'S�' ".($rs->Value('ca_colmas')=='S�'?'CHECKED':'').">S�&nbsp;&nbsp;&nbsp;&nbsp;<INPUT ID='chk_colmas_1' NAME='colmas' TYPE='radio' VALUE = 'No' ".($rs->Value('ca_colmas')=='No'?'CHECKED':'').">No</CENTER></TD>";
             echo "      <TD Class=mostrar>16. Seguro:<BR><CENTER><INPUT ID='chk_seguro_0' NAME='seguro' TYPE='radio' VALUE = 'S�' ".($rs->Value('ca_seguro')=='S�'?'CHECKED':'').">S�&nbsp;&nbsp;&nbsp;&nbsp;<INPUT ID='chk_seguro_1' NAME='seguro' TYPE='radio' VALUE = 'No' ".($rs->Value('ca_seguro')=='No'?'CHECKED':'').">No</CENTER></TD>";
             echo "      <TD Class=mostrar>17. Lib. Autom�tica:<BR><CENTER><INPUT ID=si NAME='liberacion' TYPE='radio' VALUE = 'S�' ".($lib_mem?'CHECKED':'')." DISABLED>S�&nbsp;&nbsp;&nbsp;<INPUT ID=no NAME='liberacion' TYPE='radio' VALUE = 'No' ".(!$lib_mem?'CHECKED':'')." DISABLED>No</CENTER></TD>";
             echo "      <TD Class=mostrar>Tiempo de Cr�dito:<BR><CENTER><INPUT DISABLED TYPE='TEXT' NAME='tiempocredito' VALUE='".($tm->Value('ca_diascredito')!=0?$tm->Value('ca_diascredito').' D�as':'-')."' SIZE=18 MAXLENGTH=20></CENTER></TD>";
             echo "    </TR>";
             echo "    <TR HEIGHT=5>";
             echo "      <TD Class=invertir COLSPAN=4></TD>";
             echo "    </TR>";
             ...
             echo "    <TR>";
             echo "      <TD Class=listar WIDTH=175>18.1 Continuaci�n/Viaje:";
             ...
             if (!$tm->Open("select ca_email, ca_login from vi_usuarios where ca_cargo like '%Operaciones Maritimas%'")) { // Hace un Select a la vista e uncluye un registro vacio
                 echo "<script>alert(\"".addslashes($tm->mErrMsg)."\");</script>";     // Muestra el mensaje de error
                 echo "<script>document.location.href = 'reportenegocio.php';</script>";
                 exit;
                }
             ...
             if ( strlen($rs->Value('ca_usucerrado')) == 0  ){
                 echo "<TH><INPUT Class=submit TYPE='SUBMIT' NAME='accion' VALUE='Guardar Modificaci�n' ONCLICK='validar(this);'></TH>";    // Ordena almacenar los datos ingresados
                 echo "<TH><INPUT Class=submit TYPE='SUBMIT' NAME='accion' VALUE='Nueva Versi�n' ONCLICK='validar(this);'></TH>";         // Ordena almacenar los datos ingresados
             }
             ...
             echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='boton' VALUE='Cancelar' ONCLICK='javascript:document.location.href = \"reportenegocio.php?boton=Consultar\&id=$id\"'></TH>";  // Cancela la operaci�n
             echo "<script>llenar_traficos();</script>";
...
echo "</BODY>";
        ...
        case 'Liquidar': {                                                    // Opcion para Consultar un solo registro
             if (!$rs->Open("select * from vi_reportes where ca_idreporte = $id")) {                       // Selecciona todos lo registros de la tabla Ino-Mar�timo
                 echo "<script>alert(\"".addslashes($rs->mErrMsg)."\");</script>";      // Muestra el mensaje de error
                 echo "<script>document.location.href = 'entrada.php';</script>";
                 exit; }
             $tm =& DlRecordset::NewRecordset($conn);                                       // Apuntador que permite manejar la conexi�n a la base de datos
			 ...
			 echo "      if (!confirm(\"En raz�n a que �ste embarque va a ser manejado por Coltrans USA, tenga en cuenta que en reportar deben ser desglosados como m�nimo los recargos BAF y GRI, SIN IMPORTAR que hayan sido incluidos en la venta dentro del Flete bajo el esquema All-In. �Tuvo en cuenta �ste procedimiento?\")){";
			 echo "        return (false);";
             echo "      }";
             echo "  }";
             echo "  i = 0;";
			 ...
			 echo "           alert('Error - Un Recargo en Origen est� asociado a un concepto de carga que no tiene valor de fletes!');";
             echo "           return (false);";
             echo "        }";
             echo "        check = true;";
             echo "     }";
             ...
             if ($rs->Value('ca_seguro')== "S�") {
                 ...
                 echo "      alert('El valor Asegurado no es v�lido o no puede ser igual a 0');";
                 echo "  else if (document.adicionar.primaventa.value == '' || document.adicionar.primaventa.value < 0)";
                 echo "      alert('El Porcentaje de Venta Prima no es v�lido o no puede ser igual a 0');";
                 echo "  else if (document.adicionar.obtencionpoliza.value == '' || document.adicionar.obtencionpoliza.value < 0)";
                 echo "      alert('El Valor por Obtenci�n de la P�liza no es v�lido o no puede ser igual a 0');";
                 echo "  else";
                 echo "      return (true);";
                 echo "  return (false);";
                }
             ...
             echo "<H3>$titulo</H3>";
             echo "<FORM METHOD=post NAME='adicionar' ACTION='reportenegocio.php' ONSUBMIT='return validar();'>";// Crea una forma con datos vacios
             echo "<INPUT TYPE='HIDDEN' NAME='coltrans_usa' VALUE=".(($rs->Value('ca_idagente')=='317')?'true':'false').">";                 // Condici�n especial para Coltrans MIA
             ...
             if ($rs->Value('ca_seguro')== "S�") {
                 ...
                 if (!$mn->Open("select ca_idmoneda, ca_nombre from tb_monedas order by ca_nombre")) {       // Selecciona todos lo registros de la tabla Monedas
                     ...
                     echo "<script>document.location.href = 'reportenegocio.php';</script>";
                     exit; }
				 if (!$tm->Open("select ca_email, ca_login, ca_nombre from vi_usuarios where ca_cargo like '%P�lizas%'")) {          // Selecciona los correos de la tabla Parametros
					 echo "<script>alert(\"".addslashes($tm->mErrMsg)."\");</script>";      // Muestra el mensaje de error
					 echo "<script>document.location.href = 'reportenegocio.php';</script>";
					 exit; }
                 ...
                 echo "  <TR>";
                 echo "    <TD Class=invertir COLSPAN=4 style='text-align:center; font-weight:bold;'>INFORMACI�N PARA LA ASEGURADORA</TD>";
                 echo "  </TR>";
                 echo "  <TR>";
                 ...
                 echo "    </SELECT></TD>";
                 echo "    <TD Class=listar style='vertical-align:bottom;'>20.2 Obtenci�n P�liza:<BR><INPUT TYPE='TEXT' NAME='obtencionpoliza' VALUE='".($rs->Value('ca_obtencionpoliza')+0)."' SIZE=15 MAXLENGTH=15>";
             ...
             echo "<TR>";
             echo "  <TD Class=invertir COLSPAN=5><CENTER><TABLE WIDTH=100% CELLSPACING=1>";
             if ($rs->Value('ca_transporte') == 'Mar�timo') {
                 echo "<TH>Concepto</TH>";
                 echo "<TH>Cantidad</TH>";
                 echo "<TH>T.Neta</TH>";
                 echo "<TH>T.M�nimo</TH>";
             ...
             }else if ($rs->Value('ca_transporte') == 'A�reo') {
             ...
             echo "</TR>";
             echo "<TR>";
             echo "  <TD Class=invertir COLSPAN=5 style='text-align:center; font-weight:bold;'>RELACI�N DE RECARGOS</TD>";
             echo "</TR>";
             echo "<TR>";
             echo "  <TD Class=invertir COLSPAN=5><CENTER><TABLE WIDTH=100% CELLSPACING=1>";
             echo "    <TH>Recargo en Origen</TH>";
             echo "    <TH>Aplicaci�n</TH>";
             echo "    <TH>Tipo</TH>";
             echo "    <TH>T.Neta</TH>";
             echo "    <TH>T.M�nimo</TH>";
             echo "    <TH>T.Reportar</TH>";
             ...
             echo "</TABLE>";
             echo "<TABLE CELLSPACING=10>";
             echo "<TH><INPUT Class=submit TYPE='SUBMIT' NAME='accion' VALUE='Guardar Liquidaci�n'></TH>";         // Ordena almacenar los datos ingresados
             echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='accion' VALUE='Ver Cotizaci�n' ONCLICK='window.open(\"cotizacion.php?id=".$rs->Value('ca_idcotizacion')."\",\"Cotizacion\",\"scrollbars=yes,width=800,height=600,top=200,left=150\")'></TH>";         // Ordena almacenar los datos ingresados
             echo "<TH><INPUT Class=button TYPE='BUTTON' NAME='boton' VALUE=' Regresar ' ONCLICK='javascript:document.location.href = \"reportenegocio.php?boton=Consultar\&id=$id\"'></TH>";  // Cancela la operaci�n
             echo "</TABLE>";
             echo "</FORM>";
             echo "</CENTER>";
//           echo "<P DIR='RTL'><A HREF=\"#\" ONCLICK='javascript:window.open(\"./help/$modulo.html\",\"Ayuda\",\"scrollbars=yes,width=600,height=400,top=200,left=150\")'><IMG SRC='./graficos/help.gif' border=0 ALT='Ayuda en L�nea'><BR>Ayuda</A></P>";  // Link que proporciona la Ayuda en l�nea
...
elseif (isset($accion)) {                                                       // Switch que evalua cual bot�n de comando fue pulsado por el usuario
    if ($accion == 'Guardar Modificaci�n' or $accion == 'Nueva Versi�n' or $accion == 'Guardar' or $accion == 'Borrador' or $accion == 'Reporte Nuevo'){
    ...
    switch(trim($accion)) {                                                     // Switch que evalua cual bot�n de comando fue pulsado por el usuario
        case 'Guardar Modificaci�n': {                                          // El Bot�n Guardar fue pulsado
        ...
        case 'Guardar Liquidaci�n': {                                                      // El Bot�n Guardar fue pulsado
             $id = $idreporte;
        ...
        case 'Borrador': {                                                          // El Bot�n Guardar Borrador fue pulsado
        ...
        case 'Cerrar': {                                                          // El Bot�n Guardar Borrador fue pulsado
        ...
        case 'Abrir': {                                                          // El Bot�n Guardar Borrador fue pulsado
        ...
        case 'Anular': {                                                          // El Bot�n Guardar Borrador fue pulsado
        ...
        case 'Crear Reporte AG': {                                   // El Bot�n Reporte AG fue pulsado
             /*$uploaddir  = '';*/
             ...
             if($rs->Value('ca_idconsignatario') != 0){
                 $contenido.="<TR>\n";
                 $contenido.="  <TD><B>Cliente :</B></TD>\n";
                 $contenido.="  <TD COLSPAN=3>".$rs->Value('ca_nombre_con')." Enviar Informaci�n: ".$rs->Value('ca_informar_cons')."</TD>\n";
                 $contenido.="</TR>\n";
             }
             if($rs->Value('ca_idnotify') != 0){
                 $contenido.="<TR>\n";
                 $contenido.="  <TD><B>Cliente :</B></TD>\n";
                 $contenido.="  <TD COLSPAN=3>".$rs->Value('ca_nombre_not')." Enviar Informaci�n: ".$rs->Value('ca_informar_noti')."</TD>\n";
                 $contenido.="</TR>\n";
             }
             if($rs->Value('ca_idmaster') != 0){
                 $contenido.="<TR>\n";
                 $contenido.="  <TD><B>Cliente :</B></TD>\n";
                 $contenido.="  <TD COLSPAN=3>".$rs->Value('ca_nombre_mas')." Enviar Informaci�n: ".$rs->Value('ca_informar_mast')."</TD>\n";
                 $contenido.="</TR>\n";
             }
             if($rs->Value('ca_idrepresentante') != 0){
                 $contenido.="<TR>\n";
                 $contenido.="  <TD><B>Cliente :</B></TD>\n";
                 $contenido.="  <TD COLSPAN=3>".$rs->Value('ca_nombre_rep')." Enviar Informaci�n: ".$rs->Value('ca_informar_repr')."</TD>\n";
                 $contenido.="</TR>\n";
             }

             $contenido.="<TR>\n";
             $contenido.="  <TD><B>Mercancia :</B></TD>\n";
             $contenido.="  <TD COLSPAN=3>".$rs->Value('ca_mercancia_desc')."<BR><BR>&laquo;".(($rs->value("ca_mcia_peligrosa")=='t')?"S�":"NO")." es Mercanc�a Peligrosa&raquo;</TD>\n";
             $contenido.="</TR>\n";
             $contenido.="</TABLE>\n";
             ...
             enviar_email($rs, $id_email, $_FILES);                                           // Llamado a la funci�n que envia los emails
             break;
             }
        case 'Guardar' or 'Reporte Nuevo' or 'Nueva Versi�n': {                                   // El Bot�n Guardar fue pulsado
             $id_old = $id;
             ...
             */
             ...
             if ($accion =='Reporte Nuevo' or $accion =='Nueva Versi�n') {
     ...
     if (isset($status) and $status != 'false') {
		...
		$consecutivo = $rs->Value('ca_consecutivo');
        /*echo "<script>document.location.href = '../colsys_sf/index.php/traficos/verEstatusCarga/modo/impo/ver/reporte/numreporte/$consecutivo';</script>";  // Retorna a la pantalla principal de la opci�n
		*/
		
		 echo "<script>document.location.href = '/traficos/listaStatus/modo/maritimo?reporte=$consecutivo';</script>";  // Retorna a la pantalla principal de la opci�n
		
     }else if (isset($id)) {
		echo "<script>document.location.href = 'reportenegocio.php?boton=Consultar\&id=$id';</script>";  // Retorna a la pantalla principal de la opci�n
	 }else {
		echo "<script>document.location.href = 'reportenegocio.php';</script>";  // Retorna a la pantalla principal de la opci�n
     }
   }
...
function enviar_email(&$rs, $id, &$attachment){
    ...
    $mail->AltBody = "�� Este mensaje est� en formato HTML pero el equipo no est� configurado para mostrarlo autom�ticamente. Active la opci�n HTML del men� Ver en su cliente de correo electr�nico para una correcta visualizaci�n.>>";
...
function datos_basicos(&$visible,&$rs,&$tm){
     ...
     echo "  <TD Class=titulo style='text-align:center; font-weight:bold;'>Versi�n No.:</TD>";
     echo "  <TD Class=titulo style='text-align:center; font-weight:bold;'>Cotizaci�n:</TD>";
     echo "</TR>";
     echo "<TR>";
     ...
     echo "<TR>";
     echo "  <TD Class=partir>1.&nbsp;Importaci�n</TD>";
     ...
     echo "</TR>";
     echo "<TR>";
     echo "  <TD Class=listar COLSPAN=3><B>6. Descripci�n de la Mercanc�a:</B><BR>".nl2br($rs->Value('ca_mercancia_desc'))."<BR><BR>&laquo;".(($rs->value("ca_mcia_peligrosa")=='t')?"S�":"NO")." es Mercanc�a Peligrosa&raquo;</TD>";
     echo "</TR>";
     ...
     while (!$tm->Eof()) {
        ...
        echo "    <TD Class=mostrar COLSPAN=2><B>7.3 Direcci�n:</B><BR>".$tm->Value('ca_direccion')."</TD>";
        echo "  </TR>";
        echo "  <TR>";
        echo "    <TD Class=mostrar><B>7.4 Tel�fono:</B><BR>".$tm->Value('ca_telefonos')."</TD>";
        echo "    <TD Class=mostrar><B>7.5 Fax:</B><BR>".$tm->Value('ca_fax')."</TD>";
        echo "    <TD Class=mostrar COLSPAN=2><B>7.6 Correo Electr�nico:</B><BR>".$tm->Value('ca_email')."</TD>";
        echo "  </TR>";
        echo "  <TR>";
     ...
     echo "    <TD Class=listar COLSPAN=2><B>8.3 Direcci�n:</B><BR>".str_replace ("|"," ",$rs->Value('ca_direccion_cli'))."</TD>";
     echo "  </TR>";
     echo "  <TR>";
     echo "    <TD Class=listar><B>8.4 Tel�fono:</B><BR>".$rs->Value('ca_telefonos_cli')."</TD>";
     echo "    <TD Class=listar><B>8.5 Fax:</B><BR>".$rs->Value('ca_fax_cli')."</TD>";
     echo "    <TD Class=listar COLSPAN=2><B>8.6 Correo Electr�nico:</B><BR>".$rs->Value('ca_email_cli')."</TD>";
     echo "  </TR>";
     echo "  </TABLE></TD>";
     echo "</TR>";
     echo "<TR HEIGHT=5>";
     ...
     if ($rs->Value('ca_idconsignatario') != 0){
         ...
         echo "    <TD Class=listar COLSPAN=2 WIDTH=280><B>9.1.1 Enviar Informaci�n:</B><BR><CENTER>".$rs->Value('ca_informar_cons')."<CENTER></TD>";
         echo "  </TR>";
         echo "  <TR>";
         echo "    <TD Class=listar COLSPAN=2><B>9.1.2 Contacto:</B><BR>".$rs->Value('ca_contacto_con')."</TD>";
         echo "    <TD Class=listar COLSPAN=2><B>9.1.3 Direcci�n:</B><BR>".$rs->Value('ca_direccion_con')."</TD>";
         echo "  </TR>";
         echo "  <TR>";
         echo "    <TD Class=listar><B>9.1.4 Tel�fono:</B><BR>".$rs->Value('ca_telefonos_con')."</TD>";
         echo "    <TD Class=listar><B>9.1.5 Fax:</B><BR>".$rs->Value('ca_fax_con')."</TD>";
         echo "    <TD Class=listar COLSPAN=2><B>9.1.6 Correo Electr�nico:</B><BR>".$rs->Value('ca_email_con')."</TD>";
         echo "  </TR>";
         echo "  </TABLE></TD>";
         echo "</TR>";
         echo "<TR HEIGHT=5>";
     ...
     if ($rs->Value('ca_idnotify') != 0){
         ...
         echo "    <TD Class=listar COLSPAN=2 WIDTH=280><B>9.2.1 Enviar Informaci�n:</B><BR><CENTER>".$rs->Value('ca_informar_noti')."<CENTER></TD>";
         echo "  </TR>";
         echo "  <TR>";
         echo "    <TD Class=listar COLSPAN=2><B>9.2.2 Contacto:</B><BR>".$rs->Value('ca_contacto_not')."</TD>";
         echo "    <TD Class=listar COLSPAN=2><B>9.2.3 Direcci�n:</B><BR>".$rs->Value('ca_direccion_not')."</TD>";
         echo "  </TR>";
         echo "  <TR>";
         echo "    <TD Class=listar><B>9.2.4 Tel�fono:</B><BR>".$rs->Value('ca_telefonos_not')."</TD>";
         echo "    <TD Class=listar><B>9.2.5 Fax:</B><BR>".$rs->Value('ca_fax_not')."</TD>";
         echo "    <TD Class=listar COLSPAN=2><B>9.2.6 Correo Electr�nico:</B><BR>".$rs->Value('ca_email_not')."</TD>";
         echo "  </TR>";
         echo "  </TABLE></TD>";
         echo "</TR>";
         echo "<TR HEIGHT=5>";
     ...
     if ($rs->Value('ca_idmaster') != 0){
         ...
         echo "    <TD Class=listar COLSPAN=2 WIDTH=280><B>9.3.1 Enviar Informaci�n:</B><BR><CENTER>".$rs->Value('ca_informar_mast')."<CENTER></TD>";
         echo "  </TR>";
         echo "  <TR>";
         echo "    <TD Class=listar COLSPAN=2><B>9.3.2 Contacto:</B><BR>".$rs->Value('ca_contacto_mas')."</TD>";
         echo "    <TD Class=listar COLSPAN=2><B>9.3.3 Direcci�n:</B><BR>".$rs->Value('ca_direccion_mas')."</TD>";
         echo "  </TR>";
         echo "  <TR>";
         echo "    <TD Class=listar><B>9.3.4 Tel�fono:</B><BR>".$rs->Value('ca_telefonos_mas')."</TD>";
         echo "    <TD Class=listar><B>9.3.5 Fax:</B><BR>".$rs->Value('ca_fax_mas')."</TD>";
         echo "    <TD Class=listar COLSPAN=2><B>9.3.6 Correo Electr�nico:</B><BR>".$rs->Value('ca_email_mas')."</TD>";
         echo "  </TR>";
         echo "  </TABLE></TD>";
         echo "</TR>";
         echo "<TR HEIGHT=5>";
     ...
     if ($rs->Value('ca_idrepresentante') != 0){
         ...
         echo "    <TD Class=listar COLSPAN=2 WIDTH=280><B>10.1 Enviar Informaci�n:</B><BR><CENTER>".$rs->Value('ca_informar_repr')."<CENTER></TD>";
         echo "  </TR>";
         echo "  <TR>";
         echo "    <TD Class=listar COLSPAN=2><B>10.2 Contacto:</B><BR>".$rs->Value('ca_contacto_rep')."</TD>";
         echo "    <TD Class=listar COLSPAN=2><B>10.3 Direcci�n:</B><BR>".$rs->Value('ca_direccion_rep')."</TD>";
         echo "  </TR>";
         echo "  <TR>";
         echo "    <TD Class=listar><B>10.4 Tel�fono:</B><BR>".$rs->Value('ca_telefonos_rep')."</TD>";
         echo "    <TD Class=listar><B>10.5 Fax:</B><BR>".$rs->Value('ca_fax_rep')."</TD>";
         echo "    <TD Class=listar COLSPAN=2><B>10.6 Correo Electr�nico:</B><BR>".$rs->Value('ca_email_rep')."</TD>";
         echo "  </TR>";
         echo "  </TABLE></TD>";
         echo "</TR>";
         echo "<TR HEIGHT=5>";
     ...
     echo "  <TD Class=listar COLSPAN=3>14. L�nea Transporte:<BR><B>".$rs->Value('ca_nombre')."</B></TD>";
     echo "</TR>";
     echo "<TR HEIGHT=5>";
     echo "  <TD Class=invertir COLSPAN=4></TD>";
     echo "</TR>";
     ...
     echo "  <TD Class=listar COLSPAN=2>17.&nbsp;Lib.&nbsp;Autom�tica:&nbsp;&nbsp;<B>".$rs->Value('ca_liberacion')."</B><BR>Tiempo de Cr�dito: <B>".$rs->Value('ca_tiempocredito')."</B></TD>";
     echo "</TR>";
     echo "<TR HEIGHT=5>";
     echo "  <TD Class=invertir COLSPAN=4></TD>";
     echo "</TR>";
     ...
     echo "      <TD Class=mostrar COLSPAN=4><B>".$rs->Value('ca_bodega')."</B></TD>";                  // Llena el cuadro de lista con las zonas o dep�sitos aduaneros
     echo "    </TR>";
     echo "  </TABLE></TD>";
...
function carga_arreglos(&$tm){
     ...
     echo "<INPUT TYPE='HIDDEN' NAME='atransporte' VALUE='A�reo'>";
     while ( !$tm->Eof()) {
     ...
     $tm->MoveFirst();
     while ( !$tm->Eof()) {
		if ($tm->Value('ca_tipo') == 'Coordinador Log�stico' or $tm->Value('ca_tipo') == 'Operador Multimodal'){
