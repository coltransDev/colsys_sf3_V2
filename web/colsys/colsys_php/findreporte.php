...
echo "</BODY>";
        ...
        case 'Consultar':{
			...
			$command.= "  from vi_reportes rp LEFT OUTER JOIN (select rpt.ca_consecutivo as ca_consecutivo_f, max(rpa.ca_idemail) as ca_idemail from tb_reportes rpt, tb_repstatus rpa where nullvalue(rpt.ca_usuanulado) and rpt.ca_idreporte = rpa.ca_idreporte group by ca_consecutivo) rf ON (rp.ca_consecutivo = rf.ca_consecutivo_f)";
			$command.= "  LEFT OUTER JOIN tb_repstatus ra ON (ra.ca_idemail = rf.ca_idemail), (select ca_consecutivo as ca_consecutivo_f, max(ca_idreporte) as ca_idreporte from tb_reportes where nullvalue(ca_usuanulado) group by ca_consecutivo_f) rx where rp.ca_idreporte = rx.ca_idreporte and nullvalue(rp.ca_usuanulado) and rp.ca_transporte = 'Marítimo'".$condicion." order by rp.ca_idreporte DESC";
