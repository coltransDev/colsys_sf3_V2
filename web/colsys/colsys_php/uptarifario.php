            ...
            for ($i=0; $i < count($oid_r); $i++) {
                ...
                if ($oid_r[$i] != 0) {
                    ...
                    if (!$rs->IsEmpty()) {
                        ...
                        if ($cambio!=0) {
                            if (!$rs->Open("update tb_recargos set ca_idconcepto = ".${$registro}['idconcepto'].", ca_idrecargo = ".${$registro}['idrecargo'].", ca_aplicacion = '".${$registro}['aplicacion']."', ca_fchinicio = '".${$registro}['fchinicio']."', ca_fchvencimiento = '".${$registro}['fchvencimiento']."', ca_vlrfijo = ".${$registro}['vlrfijo'].", ca_porcentaje = ".${$registro}['porcentaje'].", ca_baseporcentaje = '".${$registro}['baseporcentaje']."', ca_vlrunitario = ".${$registro}['vlrunitario'].", ca_baseunitario = '".${$registro}['baseunitario']."', ca_recargominimo = ".${$registro}['recargominimo'].", ca_idmoneda = '".${$registro}['idmoneda']."', ca_observaciones = '".${$registro}['observaciones']."', ca_fchactualizado = to_timestamp('".date("d M Y H:i:s")."', 'DD Mon YYYY hh:mi:ss'), ca_usuactualizado = '$usuario' where oid = $oid_r[$i]")) {
                                echo "<script>alert(\"".addslashes($rs->mErrMsg)."\");</script>";  // Muestra el mensaje de error
