...
*/
             ...
             echo "</TR>";
             if (!$tm->Open("select ca_nombre,ca_apellido, ca_email from vi_contactos where ca_idagente = '".$rs->Value('ca_idagente')."' and ca_impoexpo like '%".$rs->Value('ca_impoexpo')."%' and ca_transporte like '%".$rs->Value('ca_transporte')."%'")) { // Selecciona todos lo registros de la tabla Agentes
                 echo "<script>alert(\"".addslashes($tm->mErrMsg)."\");</script>";                   // Muestra el mensaje de error
                 echo "<script>document.location.href = 'entrada.php';</script>";
                 exit; }
             echo "<TR>";
...
elseif (isset($accion)) {                                                       // Switch que evalua cual botòn de comando fue pulsado por el usuario
     ...
     switch(trim($accion)) {                                                    // Switch que evalua cual botòn de comando fue pulsado por el usuario
        case 'Enviar Mensaje': {                                                // El Botón Enviar Mensaje fue pulsado
            ...
            if (!$rs->Open("insert into tb_emails (ca_idemail, ca_fchenvio, ca_usuenvio, ca_tipo, ca_idcaso, ca_from, ca_fromname, ca_cc, ca_replyto, ca_address, ca_attachment, ca_subject, ca_body) values($id_email, to_timestamp('".date("d M Y H:i:s")."', 'DD Mon YYYY HH24:mi:ss'), '$usuario', 'Rep.AéreoExterior', '$id', '$recorreo', '$remitente', '$cc', '$recorreo', '$address', '', '$asunto', '$contenido')")) {
                echo "<script>alert(\"".addslashes($rs->mErrMsg)."\");</script>";  // Muestra el mensaje de error
                echo "<script>document.location.href = 'traficos_air.php';</script>";
                exit;
            }
