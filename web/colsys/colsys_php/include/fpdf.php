...
function GetAttrib($s)
{
    // Get Attrib Content -> Coltrans
    $s=(string)$s;
    return $this->$s;
}
...
function GetStyle()
{
    // Get Font Style -> Coltrans
    return $this->FontFamily.','.$this->FontStyle.','.$this->FontSizePt;
}
...
function hex2dec($color = "#000000"){
    //Convert Hexadecimal to String with color's numbers
    $R = substr($color, 1, 2);
    $red = hexdec($R);
    $V = substr($color, 3, 2);
    $green = hexdec($V);
