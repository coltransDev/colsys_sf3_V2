...
Ext.define('FormCabControl', {
            ...
            },
            {
                ...
                listeners:{
                    click:function( obj, e, eOpts )
                    {
                        ...
                        {
                            url: '<?= url_for("falabellaAdu2/GuardarArchivoControl") ?>',
                            scope: this,
                            waitMsg: 'Uploading...',
    ...
    buttons: [{
        text: 'Importar Maritimo',
        
        handler: function() {
            
        this.up('form').getForm().submit(
        {
            url: '<?= url_for("falabellaAdu2/ProcesarArchivoControl") ?>',
            scope: this,
            waitMsg: 'Uploading...',
            success: function (formPanel, action) {
                var data = Ext.decode(action.response.responseText);
    ...
    },
    {
        ...
        this.up('form').getForm().submit(
        {
            url: '<?= url_for("falabellaAdu2/ProcesarArchivoControlAereo") ?>',
            scope: this,
            waitMsg: 'Uploading...',
            success: function (formPanel, action) {
                var data = Ext.decode(action.response.responseText);
