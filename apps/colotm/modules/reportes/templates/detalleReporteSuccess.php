...
<table width="90%" border="1" class="table1">
	...
	<tr>
		...
		<td width="11%"><?php
			if( $status->getCaIdemail() && $format!="email" ){
...
<table width="90%" border="1" class="table1">
	<tr>
        <th scope="col">Archivos</th><th style="text-align: right">
            <?
            if($format!="email")
            {
            ?>
            Adjuntar <? echo link_to( image_tag( "16x16/new.gif" ,'size=16x16' ) , url_for("reportesNegPlug/filesOtm?id=".$reporte->getCaIdreporte()) , "title='Agregar Archivos'" );?>
            <?
            }
            ?>
            </th>
...
{
?>
<div  align="center"><input value="Enviar Solicitud" type="button" onclick="enviar_solicitud()"/></div>
<?
}
?>
</div>
...
{
location.href="<?=url_for("ordenes/enviarSolicitud")?>";
}
</script>
