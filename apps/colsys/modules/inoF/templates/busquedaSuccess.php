...
include_component("inoF","panelFiltro", array("modo"=>$modo));
?>
...
<?
$url = "inoF/busqueda?modo=".$modo->getCaIdmodo()."&criterio=".$criterio;
if( $cadena ){
	$url.="&cadena=".$cadena;
}
...
<table class="tableList" width="800px" border="1" id="mainTable">
	...
	if( isset($refList) ){
        ...
        <tr style="border-bottom: #A0A0A0 solid 2px">
          <td >
            <?=link_to($referencia->getCaReferencia(), "inoF/verReferenciaExt4?modo=".$modo->getCaIdmodo()."&idmaster=".$referencia->getCaIdmaster())?>
