...
/*Ext.require([
   ...
   var store = Ext.create('Ext.data.Store', {
     ...
     proxy: {
         type: 'ajax',
         url: '<?=url_for('inoF/datosFacturas?idmaster='.$idmaster)?>',
         reader: {
             type: 'json',
             root: 'root'
         }
    ...
    listeners: {
        ...
        {            
            ...
            if(e.field=="concepto")
            {
                ...
                Ext.getCmp("combo-cosnceptos").setModo('<?=$modo->getCaIdmodo()?>',rec.data.idcomprobante);
        ...
        beforeitemcontextmenu: function(view, record, item, index, e)
        {
            ...
            if(record.data.estado<=1  && <?=($enable?"true":"false")?> )
            {
                itemCm.push({
                    ...
                    handler: function() {
                        ncompro="";
                        //alert(record.get('tipocomprobante'));
                        ...
                        if(confirm("'Esta seguro de Generar el Comprobante y contabilizarla?'"))
                        {
