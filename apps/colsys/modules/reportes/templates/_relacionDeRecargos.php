...
<table width="<?=$tipo=="local"?"600":"800"?>" border="1" id="mainTable">
	...
	<tr  style='background:#F0F0F0' >
		...
		<td >
			...
			if( $editable ){			
				...
				echo observe_field("aplicacion_".$gasto->getOid(), array('update'   => '',
															'url'      => 'reportes/observeRecargos?oid='.$gasto->getOid()."&token=".md5(time()),
		...
		<td>
			...
			if( $editable ){					
				echo link_to_remote(image_tag("16x16/delete.gif"), array("url"=>"reportes/eliminarRecargo?reporteId=".$reporteNegocio->getCaIdreporte()."&oid=".$gasto->getOid()."&tipo=".$tipo."&token=".md5(time().$gasto->getOid()), 
