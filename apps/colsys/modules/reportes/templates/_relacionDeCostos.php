...
<table width="800" border="1" id="mainTable">
	...
	<tr  style='background:#F0F0F0' >
		...
		<div id="tipo_<?=$costo->getOid()?>_div" align="center" style="display:inline" 
			...
			if( $editable ){			
				...
				echo observe_field("tipo_".$costo->getOid(), array('update'   => '',
															'url'      => 'reportes/observeCostos?reporteId='.$reporteNegocio->getCaIdReporte().'&oid='.$costo->getOid()."&token=".md5(time()),
															'with'  => "'catipo='+value",
															) );
			}											
			?>
		...
		<?					
			if( $editable ){					
				echo link_to_remote(image_tag("16x16/delete.gif"), array("url"=>"reportes/eliminarCosto?reporteId=".$reporteNegocio->getCaIdreporte()."&oid=".$costo->getOid()."&token=".md5(time().$costo->getOid()), 
