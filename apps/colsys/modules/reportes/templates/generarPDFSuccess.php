...
$pdf->Row ( array ('Reporte No.: ', $reporteNegocio->getCaConsecutivo (), 'Versión No.: ', $reporteNegocio->getCaVersion () . "/" . $reporteNegocio->numVersiones (), 'Fecha Reporte: ', Utils::fechaMes ( $reporteNegocio->getCaFchreporte () ), 'Cotización: ', $id_cotizacion ) );
...
$conceptos = $reporteNegocio->getRepTarifas ();
foreach ( $conceptos as $concepto ) {
	$pdf->Row ( array ($concepto->getConcepto ()->getCaConcepto (), $concepto->getCaCantidad (), $concepto->getCaNetatar () . " " . $concepto->getCaNetaidm (), $concepto->getCaNetamin () . " " . $concepto->getCaNetaidm (), $concepto->getCaReportartar () . " " . $concepto->getCaReportaridm (), $concepto->getCaReportarmin () . " " . $concepto->getCaReportaridm (), $concepto->getCaCobrartar () . " " . $concepto->getCaCobraridm (), $concepto->getCaCobrarmin () . " " . $concepto->getCaCobraridm () ) );
}
...
$pdf->Row ( array ('Ciudad :', "", 'Elaboró :', $usu_mem, 'Fecha:', date ( "Y-m-d", mktime ( $hor, $min, $seg, $mes, $dia, $anno ) ) . " " . date ( "h:i a", mktime ( $hor, $min, $seg, $mes, $dia, $anno ) ), 'Rep. Comercial:', $reporteNegocio->getCaLogin () ) );
//$reporteNegocio->getCaSucursal()
//,
