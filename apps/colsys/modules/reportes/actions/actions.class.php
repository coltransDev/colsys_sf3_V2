...
class reportesActions extends sfActions
{
	...
	*/	
	public function executeBusquedaReporte()
	{
		...
		$c->addAscendingOrderByColumn( ReportePeer::CA_VERSION );
		$c->setLimit( 200 );
	...
	*/
	public function executeFormReporte(){
		...
		if( $this->getRequestParameter("reporteId") ){
			$reporteNegocio = ReportePeer::retrieveByPk( $this->getRequestParameter("reporteId") );
		...
		$c = new Criteria();
		$c->add( TransportadorPeer::CA_TRANSPORTE, "A%" , Criteria::LIKE );
		$c->addAscendingOrderByColumn( TransportadorPeer::CA_NOMBRE );		
		$this->aerolineas = TransportadorPeer::doSelect( $c );	
		
		$c = new Criteria();
		$c->add( TransportadorPeer::CA_TRANSPORTE, "Mar%" , Criteria::LIKE );
		$c->addAscendingOrderByColumn( TransportadorPeer::CA_NOMBRE );		
		$this->navieras = TransportadorPeer::doSelect( $c );	
		
		$c = new Criteria();
		$c->add( TransportadorPeer::CA_TRANSPORTE, "Terrestre" );
		$c->addAscendingOrderByColumn( TransportadorPeer::CA_NOMBRE );		
		$this->transportadores = TransportadorPeer::doSelect( $c );	
...
/*		$cotProducto = $reporteNegocio->getCotProducto();		
	...
	*/
	public function executeCopiarReporte(){	
		...
		$this->redirect( "reportes/consultaReporte?modo=".$this->modo."&reporteId=".$reporteNegocio->getCaidreporte()."&token=".md5(time()) );
	...
	*/
	...
	{
		$this->forward("reportes", "formReporte");
	}
	...
	*/
	public function executeFormReporteGuardar(){
		...
		if( $this->getRequestParameter( "idconcliente") ){				
			...
			$ca_fchreporte = $this->getRequestParameter( "fchReporte" );
			if( $ca_fchreporte ){
				$reporteNegocio->setCaFchreporte( $ca_fchreporte );  
			}
				
			$reporteNegocio->setCaIdcotizacion( "''" ); 
			...
			if( $this->modo=="expo" ){
				...
				$reporteNegocio->setCaIdproveedor( "''" ); 		
			...
			***********************************************************************/
			if( $reporteNegocio->getCaImpoexpo() =="Exportación" ){				
				...
				$dimensiones_largo = floatval($this->getRequestParameter( "dim_largo" ));
				$dimensiones_ancho = floatval($this->getRequestParameter( "dim_ancho" ));
				$dimensiones_alto =  floatval($this->getRequestParameter( "dim_alto" ));
				$ca_dimensiones = $dimensiones_largo."|".$dimensiones_ancho."|".$dimensiones_alto;
				$repExpo->setCaDimensiones( $ca_dimensiones );	
	...
	*/
	public function executeConsultaReporte(){
		...
		$reporteNegocio = ReportePeer::retrieveByPk( $id_reporte );
		$this->forward404Unless( $reporteNegocio );					
	...
	*/
	public function executeGenerarPDF(){
		$this->reporteNegocio = ReportePeer::retrieveByPk( $this->getrequestparameter("reporteId") );
	...
	*/
	public function executeAnularReporte(){
		...
		$user = $this->getuser();
		$this->reporteNegocio->getCaUsuanulado( $user->getUserId() );
		$this->reporteNegocio->getCaFchanulado( date("Y-m-d H-i-s") );
		$this->reporteNegocio->save();
		
		$this->redirect( "reportes/index?modo=".$modo );
