...
if( $status ){
	$etapa = $status->getTrackingEtapa();
	
	$idetapa = $etapa?$etapa->getCaIdetapa():"";	
}	




use_helper("Object", "MimeType",  "YUICalendar");
...
if( $reporte->getCaImpoExpo()=="Importación" || $reporte->getCaImpoExpo()=="Triangulación" ){
	if( $reporte->getCaTransporte()=="Marítimo" ){
		...
		$intro ="";	
		$introAviso="";	
		$introPreaviso="Estimado cliente, nos complace informar los detalles de vuelo para el siguiente embarque:";
...
<script language="javascript">
	function cambiarAviso(){	
		...
		var etapaStr=etapa[etapa.selectedIndex].text;
		<?
		}else{		
		?>
			var etapaStr="ETA";			
...
<table width="700px" border="0" cellspacing="0" cellpadding="0" class="tableForm">
		...
		<td colspan="3"><div align="left"><strong>Etapa:</strong><br />
			...
			if( $tipo=="aviso" ){
				...
				if( $reporte->getCaimpoExpo()=="Importación"){
					if( $reporte->getCaTransporte()=="Marítimo"){						
						echo input_hidden_tag("etapa", "IMETA");
					}else{
						echo input_hidden_tag("etapa", "IAETA");
					}
				}else{
					echo input_hidden_tag("etapa", "EEETA");
				}
			}else{
				if( $reporte->getCaIdetapa()=='99999' ){ // Solo permite realizar status si ya se cerro
					echo "Status ".input_hidden_tag("etapa", "88888");
				}else{
					echo select_tag("etapa", objects_for_select( $etapas, "getCaIdetapa" , "getCaEtapa", $idetapa  ), "onChange=cambiarAviso() ");
				}
			}				
			?>			
		</div></td>
	...
	<tr>
		<td colspan="3"><div align="left"><strong>Asunto:</strong><br />
				...
				if( $reporte->getCaImpoExpo()=="Importación" || $reporte->getCaImpoExpo()=="Triangulación" ){
					$proveedor = substr($reporte->getProveedoresStr(),0,120);					
			...
			<fieldset >
				...
				?>
						<tr>
							...
							$value = $reporte->getProperty($name);
							if( $type=="date" ){
								echo yui_calendar($name, $value?$value:'', null, "2008-12-31" ); 
								echo image_tag("16x16/no.gif", array("onClick"=>"document.getElementById('".$name."').value=''") );	
							}else{
								echo input_tag($name, $value?$value:'' ); 
							}	
							?></td>
						</tr>						
				<?
...
<script language="javascript">
	...
	if( $tipo=="aviso" ){
		...
		if( $reporte->getCaimpoExpo()=="Importación"){
			if( $reporte->getCaTransporte()=="Marítimo"){										
				?>
				document.getElementById('etapa').value="IMETA";
				<?
			}else{
				?>
				document.getElementById('etapa').value="IAETA";
				<?				
			}
		}else{			
			?>
			document.getElementById('etapa').value="EEETA";
			<?
		}
	}
	?>
