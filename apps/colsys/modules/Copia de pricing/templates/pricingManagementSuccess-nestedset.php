...
Controller = function()
{
    ...
    var data = [
			...
			foreach( $trayectos as $trayecto ){				
				...
				?>
				{_id: <?=$trayecto->getCaIdtrayecto()+5000?>, company: '<?=$transportador?$transportador->getCaNombre():""?>', origen: '<?=$trayecto->getOrigen()?>', destino: '<?=$trayecto->getDestino()?>',  inicio:'<?=$trayecto->getCaFchinicio("d/m/Y")?>',vencimiento: '<?=$trayecto->getCaFchvencimiento("d/m/Y")?>', moneda:"<?=$trayecto->getCaIdMoneda()?>","price":64.72,"change":0.06,"pct_change":0.09,"last_change":"9\/1 12:00am","_parent":null,"_level":1,"_lft":<?=$i?>,"_rgt":<?=$i+1?>,"_is_leaf":false}
				<?	
				$i++;				*/
			}
			?>
