...
foreach ($columnas as $key => $value) {
    if ($key >= 0 && $key <= 1) { // Excluye A?o y Mes de los criterios de agrupamiento
        continue;
    }
...
<script type="text/javascript">
    ...
    var storeModalidad = Ext.create('Ext.data.Store', {
        ...
        proxy: {
            type: 'ajax',
            url: '<?= url_for('widgets5/datosModalidadesJSON') ?>',
            reader: {
                type: 'json',
                root: 'root'
            }
    ...
    Ext.onReady(function () {
        ...
        Ext.create('Ext.form.Panel', {
            ...
            title: 'Generador de Reportes por Deducciones',
            bodyPadding: '0, 0, 0, 0',
            layout: 'column',
            collapsible: true,
            standardSubmit: true,
            items: [{
                    ...
                    items: [{
                            ...
                            items: [{
                                    ...
                                    listeners: {
                                        change: function (comboMulti, newValue, oldValue, eOpts) {
                                            var transporte = Ext.getCmp('transporte');
                                            var modalidad = Ext.getCmp('modalidad');
                                            var deduccion = Ext.getCmp('deduccion');

                                            if (transporte.getValue().length) {
                                                modalidad.getStore().load({
                                                    params: {
                                                        'impoexpo': comboMulti.getValue().join(),
                                                        'transporte': transporte.getValue().join()
                                                    }
                                                });
                                                deduccion.getStore().load({
                                                    params: {
                                                        'impoexpo': comboMulti.getValue().join(),
                                                        'transporte': transporte.getValue().join(),
                                                        'modalidad': modalidad.getValue().join()
                                                    }
                                                });
                                            }
                                        }
                            ...
                            items: [{
                                    ...
                                    listeners: {
                                        change: function (comboMulti, newValue, oldValue, eOpts) {
                                            var impoexpo = Ext.getCmp('impoexpo');
                                            var modalidad = Ext.getCmp('modalidad');
                                            var deduccion = Ext.getCmp('deduccion');

                                            if (comboMulti.getValue().length) {
                                                modalidad.getStore().load({
                                                    params: {
                                                        'impoexpo': impoexpo.getValue().join(),
                                                        'transporte': comboMulti.getValue().join()
                                                    }
                                                });
                                                deduccion.getStore().load({
                                                    params: {
                                        ...
                                        change: function (comboMulti, newValue, oldValue, eOpts) {
                                            deduccion.getStore().load({
                                                params: {
                                                    'impoexpo': impoexpo.getValue().join(),
                                                    'transporte': comboMulti.getValue().join(),
                                                    'modalidad': comboMulti.getValue().join()
                                                }
                                            });
                                        }
                                    }
                            ...
                            store: {
                                ...
                                proxy: {
                                    type: 'ajax',
                                    url: '/widgets5/listaDeduccionesJSON',
                                ...
                                listeners: {
                                    load: function (store) {
                                        store.add({deduccion: 'Listar Todas las Deducciones'});
                                    }
                                }
                            },
                            style: 'text-align: left',
                            valueField: 'deduccion',
                            displayField: 'deduccion',
                            ddReorder: false
                        }
                    ]
                }, {
            ...
            buttons: [{
                    ...
                    handler: function () {
                        ...
                        if (form.getForm().isValid()) {
                            ...
                            panel.getForm().load({
                                // method: 'GET',
                                url: 'reporteDeduccionesListExt5',
                                params: {
                                    datos: str,
                                    filtros: str_1,
                                    columnas: str_2
                                ...
                                failure: function (form, action) {
                                    console.log(action);
                                    Ext.Msg.alert("Problemas cargando la informaci&oacute;n", action.result.errorInfo);
                                },
                                waitMsg: 'Leyendo informaci&oacute;n de la Base de Datos',
                                waitTitle: 'Cargando...'
                            });
                        }
                    }
