...
class reportesGerComponents extends sfComponents
{
	...
	*/
    ...
    public function executeFormReporteContenedores(sfWebRequest $request){
        $this->annos = array();
        for ($i = (date("Y")); $i >= (date("Y") - 5); $i--) {
            $this->annos[] = $i;
        }
        ...
        $this->continuacion = array();
        $this->continuacion[] = array("continuacion" => "%", "descripcion" => "Todos");
        $this->continuacion[] = array("continuacion" => "N/A", "descripcion" => utf8_encode("Sin Continuación"));
        $this->continuacion[] = array("continuacion" => "OTM", "descripcion" => "Carga en OTM");
        $this->continuacion[] = array("continuacion" => "DTA", "descripcion" => "Carga en DTA");
        $this->continuacion[] = array("continuacion" => "OTM/DTA", "descripcion" => "OTM o DTA");
    ...
    public function executeFormInformeAerolineasExpo() {
        list($ano, $mes) = sscanf(date("Y-m"), "%d-%d");
        
        $con = Doctrine_Manager::getInstance()->connection();
        $sql = "select id.ca_id, id.ca_nombre from ids.tb_ids id inner join public.tb_expo_carrier ec on ec.ca_idcarrier = id.ca_id order by ca_nombre";
        $rs = $con->execute($sql);
        $aerolineas_rs = $rs->fetchAll();
        $this->aerolineas = array();
        foreach ($aerolineas_rs as $aerolinea) {
            $this->aerolineas[] = array("idcarrier" => $aerolinea["ca_id"], "carrier" => utf8_encode($aerolinea["ca_nombre"]));
        }
        
        $this->fechaInicial = date("Y-m-d", mktime(0, 0, 0, $mes, 1, $ano));
        $this->fechaFinal = date("Y-m-d", mktime(0, 0, 0, $mes+1, 0, $ano));
    }
}
?>