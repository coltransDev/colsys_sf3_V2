...
$empresa = Doctrine::getTable("Empresa")->find(11); // Localiza la empresa Coldepósitos
...
foreach ($depositos as $deposito) {
    if (trim($deposito->getCaObservaciones()) != "") {
        $imprimirObservaciones[$deposito->getCosto()->getCaParametros()] = true;
    }
}

if (count($depositos) > 0) {

    $imprimirNotas[] = "deposito";

    $pdf->Ln(4);
    $pdf->SetFont($font, 'B', 9);
    $pdf->Cell(0, 4, 'COSTOS DE DEPÓSITO', 0, 1, 'L', 0);
    $pdf->SetFont($font, '', 9);
    $i = 1;
    $linea = "";
    $vigencia = array();
    ...
    foreach ($depositos as $deposito) {
        $pdf->beginGroup();
        if ($linea != $deposito->getCosto()->getCaParametros()){
            //Control impresión
            $pdf->Ln(4);
            $pdf->SetFont($font, '', 9);
            $pdf->Cell(0, 4, $deposito->getCosto()->getCaParametros(), 0, 1, 'L', 0);
            $pdf->Ln(2);
            $pdf->SetFont($font, '', 7);

            $titu_mem = array('Concepto', 'Valor');
            if ($imprimirObservaciones[$deposito->getCosto()->getCaParametros()]) {
                array_push($titu_mem, 'Observaciones');
                $width_mem = array(45, 55, 70); // = 170
            } else {
                $width_mem = array(50, 120);  // = 170
            }
            ...
            $pdf->Row($titu_mem);
            $linea = $deposito->getCosto()->getCaParametros();
        }
        ...
        $valor = "";
        if ($deposito->getCaValor() > 0 and $deposito->getCaValor() < 1) {
                $valor.= Utils::formatNumber($deposito->getCaValor())." %";
        }else if ($deposito->getCaValor() >= 1) {
                $valor.= "$ ".Utils::formatNumber($deposito->getCaValor());
        }
        if ($deposito->getCaAplicacion()){
                $valor.= " " . $deposito->getCaAplicacion();
        }
        if ($deposito->getCaValorminimo()){
            $valor.= " Mínimo :";
            if ($deposito->getCaValorminimo() > 0 and $deposito->getCaValorminimo() < 1) {
                    $valor.= Utils::formatNumber($deposito->getCaValorminimo())." %";
            }else if ($deposito->getCaValorminimo() >= 1) {
                    $valor.= "$ ".Utils::formatNumber($deposito->getCaValorminimo());
            }
            if ($deposito->getCaAplicacionminimo()){
                    $valor.= " " . $deposito->getCaAplicacionminimo();
            }
        }
		
        $row = array(
            $deposito->getCosto()->getCaCosto()." ".$deposito->getCaParametro(),
            $valor
        );
        if ($imprimirObservaciones) {
            array_push($row, $deposito->getCaObservaciones());
        }
        $pdf->Row($row);
        $pdf->flushGroup();
    }
