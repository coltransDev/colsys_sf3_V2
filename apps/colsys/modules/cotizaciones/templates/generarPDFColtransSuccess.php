...
for ($k = 0; $k < count($transportes); $k++):
   ...
   foreach ($productos as $producto):
      ...
      if ($producto->getCaImprimir() == 'Puerto'):
         ...
         foreach ($opciones as $opcion) {
            ...
            $tabla[$producto->getOrigen()->getCaCiudad()][$producto->getDestino()->getCaCiudad()][$num_mem]["fletes"] = "Flete: " . $opcion->getTextoFlete();
            $tabla[$producto->getOrigen()->getCaCiudad()][$producto->getDestino()->getCaCiudad()][$num_mem]["recargos"] = $opcion->getTextoRecargos();
         ...
         if ($producto->getCaVigencia()) {
            $tabla[$producto->getOrigen()->getCaCiudad()][$producto->getDestino()->getCaCiudad()][$num_mem]["vigencia"] = "Valida Hasta: " . Utils::fechaMes($producto->getCaVigencia());
         }
   ...
   if (count($tabla) > 0) {
      ...
      $width = round(145 / count($destinos), 0);
      ...
      $pdf->SetAligns(array_merge(array("C"), array_fill(0, count($destinos), "L")));
         ...
         for ($i = 0; $i < $max_row; $i++) {
            ...
            foreach ($destinos as $destino) {
               ...
               if (isset($concepto["concepto"])) {
                  $row_1 = array_merge($row_1, array($concepto["concepto"], $concepto["fletes"]));
               } else {
               ...
               $widths = array_merge($widths, array(round($width * 0.40, 0), round($width * 0.60, 0)));
            ...
            $pdf->SetWidths($widths);
            $pdf->SetStyles(array_merge(array("B"), array_fill(0, count($destinos) * 2, "B")));
            $pdf->Row($row_1);
            $widths = array_merge(array(25), array_fill(0, count($destinos), $width));
            $pdf->SetStyles(array_merge(array("B"), array_fill(0, count($destinos), "")));
            $pdf->SetWidths($widths);
            if (trim(implode($row_2)) != "") {
               $pdf->Row($row_2);
            }
         }
         $pdf->Row($row_3);
