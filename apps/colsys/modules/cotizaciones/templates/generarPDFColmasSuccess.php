...
//	$imprimirNotas[]="tarifas";
	...
	$pdf->Ln(4);	
	$pdf->SetFont($font,'B',9);
	$pdf->Cell(0, 4, 'TARIFAS DE ADUANA', 0, 1, 'L', 0);
	$pdf->SetFont($font,'',9);
	$pdf->Ln(2);
	$i = 1;
	$linea = "";
...
//	$titu_mem= array('Concepto','Parametro','Valor','Aplicacion','Valor Minimo','Aplicacion');
   $titu_mem= array('Concepto','Parametro','Valor','Aplicacion','Valor Minimo','Aplicacion');

   if( $imprimirObservaciones ){
		array_push( $titu_mem, 'Observaciones' );
		$width_mem= array(30, 20, 20, 20, 20 , 20, 48);
	}else{
		$width_mem= array(38, 30, 25, 30, 25 , 30);//178
	}
	...
	foreach( $tarifas as $tarifa ){
      ...
      if( $imprimirObservaciones ){
			array_push( $row,  $tarifa->getCaObservaciones() );
      }
/*      $tarifa->getCaIdconcepto(),
					$tarifa->getCaParametro()!=null?$tarifa->getCaParametro():"",
               $tarifa->getCaValor()!=null?$tarifa->getCaValor():"",
               $tarifa->getCaAplicacion()!=""?$tarifa->getCaAplicacion():"",
               $tarifa->getCaValorminimo()!=null?$tarifa->getCaValorminimo():"",
               $tarifa->getCaAplicacionminimo()!=null?$tarifa->getCaAplicacionminimo():""
 *
 */
		$pdf->Row($row);
	}
