...
Ext.Loader.setConfig({
   ...
   var store = Ext.create('Ext.data.Store', {
     ...
     proxy: {
         type: 'ajax',
         url: '<?=url_for('inoF2/datosGridCostosPanel')?>',         
         reader: {
             type: 'json',
             root: 'root',
             totalProperty: 'total'
...
Ext.define('Colsys.Ino.GridCosto', {
  ...
  alias: 'widget.wCIGCosto',
    ...
    tbar: [{
        ...
        iconCls: 'add',
        handler : function(){
            document.location = "<?=url_for("inoF/formCosto")?>?modo=6<?//= $modo->getCaIdmodo()?>&idmaster=<?=$idmaster?>";       
            ...
            items: [
              {
                    text: 'Editar',                    
                    handler: function() {
                    document.location = "<?=url_for("inoF/formCosto")?>?modo=6<?//=$modo->getCaIdmodo()?>&idinocosto="+record.get('idinocosto');
                    }
                },
                {
                    text: 'Eliminar',
