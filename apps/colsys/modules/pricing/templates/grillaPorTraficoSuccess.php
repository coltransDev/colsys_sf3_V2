...
var dataConceptos = [
                        <?
                        $i=0;
                        $conceptos = $sf_data->getRaw('conceptos'); //Debido a que se muestran las entidades HTML
...
*/
<?
$url = "pricing/datosGrillaPorTrafico?impoexpo=".utf8_encode($impoexpo)."&modalidad=".$modalidad."&transporte=".utf8_encode($transporte);
...
var gridOnBeforeedit = function(e){
    if( e.record.data.tipo=='recargo' ){
        comboRecargos.store.loadData(dataRecargos);
    }else{
        comboRecargos.store.loadData(dataConceptos);
    }
		...
		this.menu = new Ext.menu.Menu({
			...
			items: [
                ...
                ?>
                {
					...
					scope:this,
					handler: function(){    					                   
						agregarFila(this.ctxRecord, index, 'concepto');
					}
				},
                {
					text: 'Nuevo recargo',
					...
					scope:this,
					handler: function(){
						agregarFila(this.ctxRecord, index, 'recargo');
					}
				},
				{
					text: 'Estado',	
...
function agregarFila(ctxRecord, index, tipo){

    if( tipo=='recargo' ){
        var orden = ctxRecord.get("orden")+'Z';
    }else{
        var orden = 888+store.getCount();        
    }    
...
*/
var ventanaControlCambios = function( record , index ){
    win = new Ext.Window({
        width       : 400,
        height      : 200,
        closeAction :'close',
...
groupGrillaTpl =  '{text} <tpl if="values.rs[0].data[\'style\'].indexOf(\'1\')!= -1"><img src="/images/yellow_group.gif" /> </tpl>'+' <tpl if="values.rs[0].data[\'style\'].indexOf(\'2\')!= -1"><img src="/images/pink_group.gif" /> </tpl>';
