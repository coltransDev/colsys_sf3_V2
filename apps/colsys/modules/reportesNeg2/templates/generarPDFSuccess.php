...
if ($reporte->getCaImpoexpo () == Constantes::EXPO && $reporte->getCaTiporep()!="3" ) {
	...
	$pdf->SetFills ( array (0, 0, 0, 0, 0, 0, 0, 0 ) );
	$pdf->SetStyles ( array ("B", "", "B", "", "B", "", "B", "" ) );
	$pdf->Row ( array ('Agente de Aduana:', $repexpo->getSia(), ( $reporte->getCaTransporte()==Constantes::MARITIMO )?'Emision BL:':'', $repexpo->getCaEmisionbl (), ( $reporte->getCaTransporte()==Constantes::MARITIMO )?'Motonave:':'Vuelo', $repexpo->getCaMotonave () ) );
...
if($reporte->getCaTiporep()=="5")
{
    ...
    $pdf->SetFills ( array (0, 0, 0, 0, 0, 0 ) );
    $pdf->SetStyles ( array ("B", "", "B", "", "B", "", "B", "" ) );
    $pdf->Row ( array ('Do:', $reporte->getDatosJson("do"), 'Urbano:', $reporte->getDatosJson("ca_direccion"), 'Entrega:', $reporte->getDatosJson("ca_direccion2") ) );
    
    $pdf->SetWidths ( array (20, 20, 20, 20, 20, 20, 20, 20, 20, 20 ) );
    $pdf->SetFills ( array (0, 0, 0, 0, 0, 0 ) );
    $pdf->SetStyles ( array ("B", "", "B", "", "B", "", "B", "", "B", "", "B", "" ) );
    $pdf->Row ( array 
            (   'Dimensiones:', $reporte->getDatosJson("ca_dimensiones"), 'Piezas:', $reporte->getDatosJson("ca_piezas"), 
                'Peso:', $reporte->getDatosJson("ca_peso"), 'Volumen:', $reporte->getDatosJson("ca_volumen"), 
                'Valor:', $reporte->getDatosJson("ca_valor") ) );
