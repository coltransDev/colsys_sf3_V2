       ...
       listeners:{
        ...
        tbar:[
            ...
            ?>
            {
                ...
                scope: this,
                handler: function(){
                    Ext.getCmp('panel-conceptos-otm').store.removeAll();
					Ext.getCmp('panel-conceptos-otm').store.reload();
				}
            }
            <?            
...
Ext.extend(PanelConceptosOtm, Ext.grid.EditorGridPanel, {
    guardarCambios: function(a,b){
        ...
        var store = Ext.getCmp('panel-conceptos-otm').store;
    ...
    onSelect : function(combo,record,index){
        ...
        {            
            ...
            var storeGrid = Ext.getCmp('panel-conceptos-otm').store;
    ...
    eliminarItem : function(){
        if( this.ctxRecord && this.ctxRecord.data.iditem && confirm("Desea continuar?") ){
            ...
            var storeConceptosOtm = this.store;
            Ext.Ajax.request(
            {
                waitMsg: 'Eliminando...',
                ...
                success:function(response,options){
                    var res = Ext.util.JSON.decode( response.responseText );
                    if( res.success ){
                        record = storeConceptosOtm.getById( res.id );
                        storeConceptosOtm.remove(record);
                        if(record.data.tipo=="concepto"){
                            ...
                            */
                            storeConceptosOtm.each( function( r ){                                
                                if( r.data.tipo=="recargo" && r.data.idconcepto==record.data.iditem ){
                                    
                                    storeConceptosOtm.remove(r);
                                }
                            });
                        }
                    }
