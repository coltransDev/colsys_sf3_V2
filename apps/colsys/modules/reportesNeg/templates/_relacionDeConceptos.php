...
if( count($conceptos)>0 || $editable ){
	echo input_hidden_tag("clicked_record_conceptos"); 
?>

<script language="javascript">
	function actualizarConceptos(btn, text){        		
		//llamado remoto
		var id = document.getElementById("clicked_record_conceptos").value;
		document.getElementById("observacion_"+id).value = text;
		<?
		echo remote_function( array('url'      => 'reportesNeg/observeConceptos?reporteId='.$reporteNegocio->getCaIdreporte()."&token=".md5(time()), 
															'with'  => "'oid='+id+'&observaciones='+document.getElementById('observacion_'+id).value",
															) );	
		?>	
	...
	function observacionConcepto( id ){
		var observacion = document.getElementById("observacion_"+id);
		document.getElementById("clicked_record_conceptos").value = id;
		Ext.MessageBox.show({
           title: 'Observaciones',
	...
	<tr  style='background:#F0F0F0' >
		...
		<td >
			...
			if( $editable ){			
				...
				echo observe_field("cantidad_".$concepto->getOid(), array('update'   => '',
															'url'      => 'reportesNeg/observeConceptos?reporteId='.$reporteNegocio->getCaIdreporte().'&oid='.$concepto->getOid()."&token=".md5(time()),
															'with'  => "'cantidad='+value",
															) );
			}											
			?>	
		...
		<div id="neta_idm_<?=$concepto->getOid()?>_div" align="center" style="display:inline" 
			...
			if( $editable ){			
				...
				echo observe_field("neta_idm_".$concepto->getOid(), array('update'   => '',
															'url'      => 'reportesreportesNeg/observeConceptos?reporteId='.$reporteNegocio->getCaIdreporte().'&oid='.$concepto->getOid()."&token=".md5(time()),
															'with'  => "'neta_idm='+value",
															) );
			}											
			?>		
		...
		<td>
			<div id="reportar_tar_<?=$concepto->getOid()?>_div" align="center" style="display:inline" 
				...
				?>
				>
				<?=Utils::formatNumber($concepto->getCaReportartar())?>
			</div>
			<?
			if( $editable ){			
				?>
		...
		<td>
			<div id="cobrar_tar_<?=$concepto->getOid()?>_div" align="center" style="display:inline" 
				...
				?>
				>
				<?=Utils::formatNumber($concepto->getCaCobrartar())?>
			</div>
			<?
			if( $editable ){			
				?>
		...
		<td>
			...
			if( $editable ){					
				echo link_to_remote(image_tag("16x16/delete.gif"), array("url"=>"reportesNeg/eliminarConcepto?reporteId=".$reporteNegocio->getCaIdreporte()."&oid=".$concepto->getOid()."&token=".md5(time().$concepto->getOid()), 
																	"update"=>"embarque" ,
																	"script"=>true,
																	"confirm"=>"Esta seguro que desea eliminar este item?",			
																	));
				echo input_hidden_tag( "observacion_".$concepto->getOid(), $concepto->getCaObservaciones() );																	
				echo image_tag("22x22/inline_table.gif", "onclick=observacionConcepto('".$concepto->getOid()."')");													
