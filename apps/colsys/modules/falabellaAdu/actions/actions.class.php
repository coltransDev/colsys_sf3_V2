	...
	*/
    public function executeGeneraFactura() {
        ...
        while ( $row = $stmt->fetch() ) {
            ...
            $salida.= "900017447 "; // 4
            $salida.= "8"; // 5
            $salida.= str_pad($row["ca_numdocumento"],10,"0", STR_PAD_LEFT); // 6
...
/*
                if($vlr_exento != 0){
                    ...
                    $adicion.= "830003960"; // 2
                    $adicion.= "900017447 "; // 3
                    $adicion.= str_pad($row["ca_numdocumento"],10,"0", STR_PAD_LEFT); // 4
...
*/
        ...
        while ( $row = $stmt->fetch() ) {
            ...
            if (!isset($acumula[$row["ca_iddoc"]])){
                ...
                $adicion.= "830003960"; // 2
                $adicion.= "900017447 "; // 3
                $adicion.= str_pad($row["ca_numdocumento"],7,"0", STR_PAD_LEFT); // 4
            ...
            $salida.= "830003960"; // 2
            $salida.= "900017447 "; // 3
            $salida.= str_pad($row["ca_numdocumento"],7,"0", STR_PAD_LEFT); // 4
