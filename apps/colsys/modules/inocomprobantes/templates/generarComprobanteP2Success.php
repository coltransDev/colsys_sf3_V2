...
//        exit;
        ...
        $datosJson= json_decode($comprobante->getCaDatos());
        
        //echo "<pre>";print_r($datosJson);echo "</pre>";
        //exit;
        if(is_array($datosJson->respuesta))
            $datosJson=$datosJson->respuesta[0];
        //echo "<pre>";print_r($datosJson);echo "</pre>";
        //exit;
        $pdf->Cell(0, 4, strtoupper($tipo->getCaTitulo())." ". $datosJson->CodigoDoc/*str_pad($comprobante->getCaConsecutivo(), 14, "0", STR_PAD_LEFT)*/,0,1, "L");
...
//        //Encabezado
        ...
        $pdf->SetXY($x+136+$aumentox,$y);
        $pdf->Cell(0, 4, "No. FACTURA PROV. "  ,0,1, "L");
        $y-=5;
        $y+=$space;
        $pdf->SetXY($x+2,$y);
        ...
        */
        ...
        $pdf->Cell(0, 4, $comprobante->getCaConsecutivo()  ,0,1, "L");
