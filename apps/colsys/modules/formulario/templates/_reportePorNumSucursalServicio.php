...
foreach($grid as $servicio =>$gridSucursal){
    ksort($gridSucursal);
    foreach($gridSucursal as $sucursal => $gridCliente){
        foreach($gridCliente as $cliente => $valor){
            $dataIni[$servicio][$sucursal]+=$valor;
        }
    }
}

foreach ($dataIni as $servicio => $dataSucursal) {
    ksort($dataSucursal);
    foreach ($serieX as $suc) {
        if ($dataSucursal[$suc])
            $data[$servicio]["data"][] = $dataSucursal[$suc];
        else
...
<div id="grafica1_1" class="bigbutton"  style="width: 1050px; height: 350px; margin: 0 auto; margin-bottom:15px; margin-top: 15px;"></div>
    ...
    $(function() {
        ...
        $(document).ready(function() {
            chart = new Highcharts.Chart({
                ...
                title: {
                    text: 'Total de Encuestas Recibidas x Sucursal y Servicio'
                },
                subtitle: {
