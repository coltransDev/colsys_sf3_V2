...
abstract class BaseCotOpcion extends BaseObject  implements Persistent {
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->aCotProducto !== null) {
				if ($this->aCotProducto->isModified()) {
	 ...
	 * @throws     PropelException
	 */
	public function setCotProducto($v)
	{
	 ...
	 * Otherwise if this CotOpcion has previously
	 * been saved, it will retrieve related CotRecargos from storage.
	 * If this CotOpcion is new, it will return
		...
		if ($this->collCotRecargos === null) {
			if ($this->isNew()) {
			   $this->collCotRecargos = array();
				...
				$criteria->add(CotRecargoPeer::CA_IDOPCION, $this->getCaIdopcion());
	 ...
	 */
	public function getCotRecargosJoinTipoRecargo($criteria = null, $con = null)
	{
		// include the Peer class
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collCotRecargos === null) {
			if ($this->isNew()) {
				...
				$this->collCotRecargos = CotRecargoPeer::doSelectJoinTipoRecargo($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
