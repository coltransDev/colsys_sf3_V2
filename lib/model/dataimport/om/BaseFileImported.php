...
/**
 * Base class that represents a row from the 'tb_fileimported' table.
 *
 * 
 *
...
abstract class BaseFileImported extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the ca_fchimportacion field.
	 * @var        int
	 */
	...
	/**
	 * Get the [optionally formatted] [ca_fchimportacion] column value.
	 * 
	...
	public function getCaFchimportacion($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->ca_fchimportacion === null || $this->ca_fchimportacion === '') {
			return null;
		} elseif (!is_int($this->ca_fchimportacion)) {
			// a non-timestamp value was set externally, so we convert it
			$ts = strtotime($this->ca_fchimportacion);
			if ($ts === -1 || $ts === false) { // in PHP 5.1 return value changes to FALSE
				throw new PropelException("Unable to parse value of [ca_fchimportacion] as date/time value: " . var_export($this->ca_fchimportacion, true));
			}
		} else {
			$ts = $this->ca_fchimportacion;
		}
		if ($format === null) {
			return $ts;
	...
	/**
	 * Set the value of [ca_fchimportacion] column.
	 * 
	 * @param      int $v new value
	 * @return     void
	 */
	public function setCaFchimportacion($v)
	{
		...
		if ($v !== null && !is_int($v)) {
			$ts = strtotime($v);
			if ($ts === -1 || $ts === false) { // in PHP 5.1 return value changes to FALSE
				throw new PropelException("Unable to parse date/time value for [ca_fchimportacion] from input: " . var_export($v, true));
			}
		} else {
			$ts = $v;
		}
		if ($this->ca_fchimportacion !== $ts) {
			$this->ca_fchimportacion = $ts;
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->ca_fchimportacion = $rs->getTimestamp($startcol + 1, null);

			$this->ca_content = $rs->getString($startcol + 2);

			$this->ca_usuario = $rs->getString($startcol + 3);

			$this->ca_procesado = $rs->getBoolean($startcol + 4);

			$this->ca_nombre = $rs->getString($startcol + 5);
			...
			return $startcol + 6; // 6 = FileImportedPeer::NUM_COLUMNS - FileImportedPeer::NUM_LAZY_LOAD_COLUMNS).
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of FileImported (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setCaFchimportacion(NULL); // this is a pkey column, so set to default value
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     FileImported Clone of current object.
	 * @throws     PropelException
	 */
