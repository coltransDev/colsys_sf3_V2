...
/**
 * Base class that represents a row from the 'control.tb_notificaciones' table.
 *
 * 
 *
...
abstract class BaseNotificacion extends BaseObject  implements Persistent {
  ...
  const PEER = 'NotificacionPeer';
	...
	/**
	 * The value for the ca_limite field.
	 * @var        string
	 */
	protected $ca_limite;
	...
	/**
	 * Get the [optionally formatted] temporal [ca_limite] column value.
	 * 
	 *
	 ...
	 */
	public function getCaLimite($format = 'Y-m-d H:i:s')
	{
		if ($this->ca_limite === null) {
			return null;
		}
		...
		try {
			$dt = new DateTime($this->ca_limite);
		} catch (Exception $x) {
			throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->ca_limite, true), $x);
		}
	...
	/**
	 * Sets the value of [ca_limite] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     Notificacion The current object (for fluent API support)
	 */
	public function setCaLimite($v)
	{
		...
		if ( $this->ca_limite !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->ca_limite !== null && $tmpDt = new DateTime($this->ca_limite)) ? $tmpDt->format('Y-m-d\\TH:i:sO') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d\\TH:i:sO') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->ca_limite = ($dt ? $dt->format('Y-m-d\\TH:i:sO') : null);
				$this->modifiedColumns[] = NotificacionPeer::CA_LIMITE;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->ca_limite = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->ca_leido = ($row[$startcol + 6] !== null) ? (boolean) $row[$startcol + 6] : null;
			...
			$this->ca_idemail = ($row[$startcol + 9] !== null) ? (int) $row[$startcol + 9] : null;
			$this->resetModified();
			...
			return $startcol + 10; // 10 = NotificacionPeer::NUM_COLUMNS - NotificacionPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			...
			$keys[5] => $this->getCaLimite(),
			$keys[6] => $this->getCaLeido(),
			$keys[7] => $this->getCaFchcreado(),
			$keys[8] => $this->getCaUsucreado(),
			$keys[9] => $this->getCaIdemail(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[5], $arr)) $this->setCaLimite($arr[$keys[5]]);
		if (array_key_exists($keys[6], $arr)) $this->setCaLeido($arr[$keys[6]]);
		...
		if (array_key_exists($keys[9], $arr)) $this->setCaIdemail($arr[$keys[9]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(NotificacionPeer::CA_LIMITE)) $criteria->add(NotificacionPeer::CA_LIMITE, $this->ca_limite);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setCaLimite($this->ca_limite);
