...
abstract class BasePricRecargosxLineaLog extends BaseObject  implements Persistent {
  ...
  const PEER = 'PricRecargosxLineaLogPeer';
    ...
    foreach (sfMixer::getCallables('BasePricRecargosxLineaLog:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePricRecargosxLineaLog:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePricRecargosxLineaLog:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BasePricRecargosxLineaLog:save:post') as $callable)
    {
  ...
  {
    if (!$callable = sfMixer::getCallable('BasePricRecargosxLineaLog:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BasePricRecargosxLineaLog::%s', $method));
    }
