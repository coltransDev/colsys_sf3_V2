...
/**
 * Base class that represents a row from the 'tb_pricpatios' table.
 *
 * 
 *
...
abstract class BasePricPatio extends BaseObject  implements Persistent {
  ...
  const PEER = 'PricPatioPeer';
	...
	/**
	 * Initializes internal state of BasePricPatio object.
	 * @see        applyDefaults()
	 */
	 ...
	 * @return     PricPatio The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = PricPatioPeer::NUM_COLUMNS - PricPatioPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$this->setCaIdpatio($pk);  //[IMV] update autoincrement primary key
	...
	/**
	 * Generic method to set the primary key (ca_idpatio column).
	 *
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of PricPatio (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setCaIdpatio(NULL); // this is a auto-increment column, so set to default value
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     PricPatio Clone of current object.
	 * @throws     PropelException
	 */
	...
	public function getCiudad(PropelPDO $con = null)
	{
		if ($this->aCiudad === null && (($this->ca_idciudad !== "" && $this->ca_idciudad !== null))) {
			...
			/* The following can be used additionally to
			   ...
			   $this->aCiudad->addPricPatios($this);
			 */
		}
		return $this->aCiudad;
	}
	 ...
	 * Otherwise if this PricPatio has previously been saved, it will retrieve
	 * related PricPatioLineas from storage. If this PricPatio is new, it will return
	 ...
	 * Otherwise if this PricPatio is new, it will return
	 * an empty collection; or if this PricPatio has previously
	 * been saved, it will retrieve related PricPatioLineas from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in PricPatio.
	 */
