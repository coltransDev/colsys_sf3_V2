...
abstract class BasePricRecargoxConceptoLog extends BaseObject  implements Persistent {
  ...
  const PEER = 'PricRecargoxConceptoLogPeer';
    ...
    foreach (sfMixer::getCallables('BasePricRecargoxConceptoLog:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePricRecargoxConceptoLog:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePricRecargoxConceptoLog:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BasePricRecargoxConceptoLog:save:post') as $callable)
    {
  ...
  {
    if (!$callable = sfMixer::getCallable('BasePricRecargoxConceptoLog:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BasePricRecargoxConceptoLog::%s', $method));
    }
