...
abstract class BasePricRecargosxCiudad extends BaseObject  implements Persistent {
  ...
  const PEER = 'PricRecargosxCiudadPeer';
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = PricRecargosxCiudadPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
    ...
    foreach (sfMixer::getCallables('BasePricRecargosxCiudad:delete:pre') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePricRecargosxCiudad:delete:post') as $callable)
    {
    ...
    foreach (sfMixer::getCallables('BasePricRecargosxCiudad:save:pre') as $callable)
    {
      ...
      if (is_int($affectedRows))
      {
		...
		try {
			...
			$con->commit();
    foreach (sfMixer::getCallables('BasePricRecargosxCiudad:save:post') as $callable)
    {
      call_user_func($callable, $this, $con, $affectedRows);
    }

			PricRecargosxCiudadPeer::addInstanceToPool($this);
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
			throw $e;
  ...
  {
    if (!$callable = sfMixer::getCallable('BasePricRecargosxCiudad:'.$method))
    {
      throw new sfException(sprintf('Call to undefined method BasePricRecargosxCiudad::%s', $method));
    }
