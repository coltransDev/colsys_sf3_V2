...
/**
 * Base static class for performing query and update operations on the 'tb_bodegas' table.
 *
 * 
 *
...
abstract class BaseBodegaPeer {
	...
	private static $fieldNames = array (
		...
		BasePeer::TYPE_COLNAME => array (BodegaPeer::CA_IDBODEGA, BodegaPeer::CA_NOMBRE, BodegaPeer::CA_TIPO, BodegaPeer::CA_TRANSPORTE, ),
	...
	private static $fieldKeys = array (
		...
		BasePeer::TYPE_COLNAME => array (BodegaPeer::CA_IDBODEGA => 0, BodegaPeer::CA_NOMBRE => 1, BodegaPeer::CA_TIPO => 2, BodegaPeer::CA_TRANSPORTE => 3, ),
	...
	{
		return BasePeer::getMapBuilder('lib.model.public.map.BodegaMapBuilder');
	}
	/**
	 ...
	 * @param      string $column The column name for current table. (i.e. BodegaPeer::COLUMN_NAME).
	 * @return     string
	 */
	...
	const COUNT = 'COUNT(tb_bodegas.CA_IDBODEGA)';
	const COUNT_DISTINCT = 'COUNT(DISTINCT tb_bodegas.CA_IDBODEGA)';
	...
	public static function doCount(Criteria $criteria, $distinct = false, $con = null)
	{
		...
		if ($distinct || in_array(Criteria::DISTINCT, $criteria->getSelectModifiers())) {
			$criteria->addSelectColumn(BodegaPeer::COUNT_DISTINCT);
		} else {
			$criteria->addSelectColumn(BodegaPeer::COUNT);
		}
		...
		$rs = BodegaPeer::doSelectRS($criteria, $con);
		if ($rs->next()) {
			return $rs->getInt(1);
		} else {
	...
	{
		return BodegaPeer::populateObjects(BodegaPeer::doSelectRS($criteria, $con));
	}
	/**
  ...
  static public function getUniqueColumnNames()
  {
	...
	/**
	 * Method perform an INSERT on the database, given a Bodega or Criteria object.
	 *
	 * @param      mixed $values Criteria or Bodega object containing data that is used to create the INSERT statement.
	 * @param      Connection $con the connection to use
	...
	public static function doInsert($values, $con = null)
	{
		...
		if ($values instanceof Criteria) {
			...
			$criteria = $values->buildCriteria(); // build Criteria from Bodega object
		}

		$criteria->remove(BodegaPeer::CA_IDBODEGA); // remove pkey col since this table uses auto-increment
	...
	/**
	 * Method perform an UPDATE on the database, given a Bodega or Criteria object.
	 *
	 * @param      mixed $values Criteria or Bodega object containing data that is used to create the UPDATE statement.
	...
	/**
	 * Method to DELETE all rows from the tb_bodegas table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	...
	/**
	 * Method perform a DELETE on the database, given a Bodega or Criteria object OR a primary key value.
	 *
	 * @param      mixed $values Criteria or Bodega object or primary key or array of primary keys
	...
	/**
	 * Validates all modified columns of given Bodega object.
	 * If parameter $columns is either a single column name or an array of column names
	 * than only those columns are validated.
	 *
	 * NOTICE: This does not apply to primary or foreign keys for now.
	 *
	 * @param      Bodega $obj The object to validate.
...
if (Propel::isInit()) {
	...
	Propel::registerMapBuilder('lib.model.public.map.BodegaMapBuilder');
}
