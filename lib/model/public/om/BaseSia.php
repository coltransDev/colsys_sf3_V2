...
/**
 * Base class that represents a row from the 'tb_sia' table.
 *
 * 
 *
...
abstract class BaseSia extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the ca_tel field.
	 * @var        string
	 */
	protected $ca_tel;
	...
	/**
	 * Set the value of [ca_tel] column.
	 * 
	 * @param      string $v new value
	 * @return     void
	 */
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->ca_idsia = $rs->getInt($startcol + 0);

			$this->ca_nombre = $rs->getString($startcol + 1);

			$this->ca_tel = $rs->getString($startcol + 2);

			$this->ca_contacto = $rs->getString($startcol + 3);
			...
			return $startcol + 4; // 4 = SiaPeer::NUM_COLUMNS - SiaPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function delete($con = null)
	{
		...
		if ($con === null) {
			$con = Propel::getConnection(SiaPeer::DATABASE_NAME);
		}
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$this->setCaIdsia($pk);  //[IMV] update autoincrement primary key
	...
	/**
	 * Generic method to set the primary key (ca_idsia column).
	 *
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Sia (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setCaIdsia(NULL); // this is a pkey column, so set to default value
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Sia Clone of current object.
	 * @throws     PropelException
	 */
