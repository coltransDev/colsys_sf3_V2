...
/**
 * Base class that represents a row from the 'tb_monedas' table.
 *
 * 
 *
...
abstract class BaseMoneda extends BaseObject  implements Persistent {
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			$this->ca_idmoneda = $rs->getString($startcol + 0);

			$this->ca_nombre = $rs->getString($startcol + 1);

			$this->ca_referencia = $rs->getString($startcol + 2);
			...
			return $startcol + 3; // 3 = MonedaPeer::NUM_COLUMNS - MonedaPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	/**
	 * Generic method to set the primary key (ca_idmoneda column).
	 *
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setCaIdmoneda(NULL); // this is a pkey column, so set to default value
	 ...
	 * Otherwise if this Moneda has previously
		...
		if ($this->collCotSeguros === null) {
			if ($this->isNew()) {
			   $this->collCotSeguros = array();
				...
				$criteria->add(CotSeguroPeer::CA_IDMONEDA, $this->getCaIdmoneda());
	 ...
	 */
	public function getCotSegurosJoinCotizacion($criteria = null, $con = null)
	{
		// include the Peer class
		if ($criteria === null) {
			$criteria = new Criteria();
		...
		if ($this->collCotSeguros === null) {
			if ($this->isNew()) {
				...
				$this->collCotSeguros = CotSeguroPeer::doSelectJoinCotizacion($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
