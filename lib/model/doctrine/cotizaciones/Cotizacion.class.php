...
class Cotizacion extends BaseCotizacion {
    ...
    public function enBlanco() {
        ...
        $count+=count($aduanas);
        $deposito = $this->getCotDeposito();
        $count+=count($deposito);
        return $count == 0;
    }
    ...
    public function getDirectorioBase() {
        $folder = Cotizacion::FOLDER;
        return $directory = $folder . DIRECTORY_SEPARATOR . strtr($this->getCaEmpresa(), "áéíóú", "aeiou") .DIRECTORY_SEPARATOR .substr($this->getCaConsecutivo(), -4).DIRECTORY_SEPARATOR . $this->getCaConsecutivo();
    }
