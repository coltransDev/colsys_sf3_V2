...
class JTree {
     ...
     * 
     * Retreives the recursive array 
     * 
     * @return array
     */
    public function getTreeNodes() {
        $array_tree = $this->getTreeChildren($this->_head);
     ...
     * Author:: Carlos G. López M.
     * 
     * Retreives the children array by node
     * 
     * @return array
     */
    public function getTreeChildren($uid) {
        if (empty($uid)) {
        ...
        foreach ($node->getChildren() as $children) {
            ...
            foreach ($child->getAttributes() as $key => $value) {
                ...
                $ifExpanded = $key == "expanded"?true:null;                
            }

            $attrib["uid"] = $uid;
            $attrib["text"] = $this->getValue($children).$count;
            if($ifExpanded)
                $attrib["expanded"] = $attrib["expanded"];
            if($this->getTreeChildren($children))
     ...
     * 
     * This is a helper function to get the 
     * uid node if it was found, false else
     * 
     * @param mixed $uid
     * @return string $uid / false
     */
    public function findValue($uid, $value = null) {
        if (empty($uid)) {
            throw new Exception('A unique ID is required.');
        }
        $node = $this->getNode($uid);
        ...
        foreach ($node->getChildren() as $children) {
            if ($this->getValue($children) == $value) {
                return $children;
            }
        }
        return false;
